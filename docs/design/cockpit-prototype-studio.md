...
| Artifact throws during render (crash) | `cc-app-error` beacon → `AppFrameLayer.tsx` marks the slot crashed, shows `.embed-app-crashed`. **In Studio context specifically, no reload button is rendered in the crashed strip** (`isStudioHost` gate in `AppFrameLayer.tsx` — `AppReloadButton`/`AppPinButton`/`AppFullscreenButton` are non-studio-only this phase); recovery requires dispatching the same `cockpit:app-reload` window `CustomEvent` the (elsewhere-rendered) reload button itself dispatches. Documented gap, not fixed this phase. |
