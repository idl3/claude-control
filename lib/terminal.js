...
// xterm.js font size for the passthrough. Smaller than the default (15) so the
// attached tmux pane fits more columns → less agent-TUI wrapping → robust
// question parsing. Overridable per device. ponytail: a const + env knob, not a
// settings UI — bump CLAUDE_CONTROL_TERM_FONT if a screen needs different.
const TTYD_FONT_SIZE = Number(process.env.CLAUDE_CONTROL_TERM_FONT) || 12;
