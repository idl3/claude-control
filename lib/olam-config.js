 ...
 *       {
 *         "org": "example",
 *         "runnerUrl": "https://<runner>.<subdomain>.workers.dev",
 *         "spaBase": "https://<spa-host>",
 *         "gsmProject": "<gcp-project>",  // optional; else CLAUDE_CONTROL_GSM_PROJECT
 *         "gsmAccount": "<gcloud-account>", // optional; else CLAUDE_CONTROL_GSM_ACCOUNT
 *         "runnerTokenGsmSecret": "<secret-name>", // optional (default: olam-<org>-sandbox-runner-token)
 *         "runnerTokenFiles": ["~/.olam/secrets/<file>"] // optional fallbacks, tried after GSM
 *       }
 *     ]
 *   }
 *
 * Endpoints, project ids and account names are OPERATOR DATA and live only in
 * this file on the operator's machine — never in the repo. With no GSM project
 * or account resolvable, the GSM candidate is skipped and token files are used.
 *
...
// Env lookup: CLAUDE_CONTROL_<X> only (mirrors server.js / lib/config.js). The
// pre-rename legacy alias was removed.
const env = (name) => process.env[`CLAUDE_CONTROL_${name}`];

/** Resolve the data directory — delegates to lib/config.js's shared resolver
 * (CLAUDE_CONTROL_DATA override, else ~/.claude-control with legacy ~/.cockpit
 * migration). */
function dataDir() {
  return getDataDir();
}

// No baked-in GCP identity: env first, else the org's own olam.json fields,
// else null (runnerTokenCandidates then skips the GSM candidate entirely).
const DEFAULT_GSM_PROJECT = env('GSM_PROJECT') ?? null;
const DEFAULT_GSM_ACCOUNT = env('GSM_ACCOUNT') ?? null;
...
export function runnerTokenCandidates(orgCfg) {
  // No project/account → nothing to query GSM with; fall straight to files
  // rather than shelling out to gcloud with undefined flags.
  const gsm =
    orgCfg.gsmProject && orgCfg.gsmAccount
      ? [
          {
            kind: 'gsm',
