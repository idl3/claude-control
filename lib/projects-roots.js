 ...
 * then every existing sibling `~/.claude-*` directory's `projects` subdir.
 *
 * A sibling qualifies only if `<homeDir>/.claude-*` has a `projects` subdir
 * that is itself a directory — which naturally excludes the cockpit's own
 * data dir `~/.claude-control` (it holds media/logs, not projects). We ALSO
 * hard-exclude `~/.claude-control` and an optional `dataDir` defensively so
 * the server never scans its own state.
 *
