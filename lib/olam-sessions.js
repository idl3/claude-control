...
export class RemoteSessionSource {
  ...
  constructor(olamConfig, registry, deps = {}) {
    ...
    this.orgs = olamConfig.orgs.map((orgCfg) => {
      ...
      return { cfg: orgCfg, client, probe, lastRows: [], capped: false };
    });
  }
  ...
  async _fetchOrg(entry) {
    ...
    try {
      ...
      entry.lastRows = rows.map((r) => {
        ...
        // archived is derived from the row's OWN (now-merged) fields, so it
        // must be computed after the spread above, not before.
   ...
   * lapsed Access session before any session was ever fetched). `capped`
   * signals the last listSessions() page hit LIST_SESSIONS_LIMIT — the org
   * may have more sessions than the tab's count reflects.
   */
  health() {
    return Object.fromEntries(
      this.orgs.map((o) => [o.cfg.org, { ...o.probe.state, capped: o.capped }]),
    );
  }
