...
export function summarizeClientPerf(limit = MAX_RECENT_BATCHES) {
  ...
  return {
    ...
    summary: {
      ...
      maxVisibleIframes: max(visible.map((s) => s.surfaces?.visibleIframes)),
      maxVisibleCanvases: max(visible.map((s) => s.surfaces?.visibleCanvases)),
      maxRunningAnimations: max(visible.map((s) => s.surfaces?.runningAnimations)),
      maxPlayingVideos: max(visible.map((s) => s.surfaces?.playingVideos)),
      maxPlayingAudio: max(visible.map((s) => s.surfaces?.playingAudio)),
      voiceActiveSamples: visible.filter((s) => s.surfaces?.voiceActive).length,
...
function normalizeSample(s) {
  ...
  return {
    ...
    wsKbPerSec: finiteOrZero(s.wsKbPerSec),
      ...
      surfaces: normalizeSurfaces(s.surfaces),
      visibility: s.visibility === 'hidden' ? 'hidden' : 'visible',
  };
}

function normalizeSurfaces(s = {}) {
  return {
    iframes: finiteOrZero(s.iframes),
    visibleIframes: finiteOrZero(s.visibleIframes),
    videos: finiteOrZero(s.videos),
    playingVideos: finiteOrZero(s.playingVideos),
    audios: finiteOrZero(s.audios),
    playingAudio: finiteOrZero(s.playingAudio),
    canvases: finiteOrZero(s.canvases),
    visibleCanvases: finiteOrZero(s.visibleCanvases),
    runningAnimations: finiteOrZero(s.runningAnimations),
    embedHoists: finiteOrZero(s.embedHoists),
    visibleEmbedHoists: finiteOrZero(s.visibleEmbedHoists),
