// lib/tui.js — parse the Claude Code TUI status line from a capture-pane dump.
//
// The bottom of a Claude session renders a status line such as:
//   /claude-cockpit Opus 4.8 (1M context) ctx:35%      Remote Control active
// and a title rule line such as:
//   ───────────────────── auto-cleanup-uploads ──
// We extract the model label, the context-remaining percentage, and whether
// Claude is actively generating ("esc to interrupt" working line). All fields
// are optional — older/narrower panes may omit them.

const ANSI_RE = /\x1b\[[0-9;]*m/g;
const CTX_RE = /ctx:\s*(\d+)\s*%/i;
const MODEL_RE = /\b(Opus|Sonnet|Haiku)\s+[\d.]+(?:\s*\([^)]*\))?/i;
const EFFORT_RE = /\beffort:(\w+)/i;
// Claude is actively generating when the TUI shows the working line. Two signals:
//  1) "esc to interrupt" (the classic interruptible working line), OR
//  2) the loader+timer: a verb ending in "…" followed by a "(Ns" elapsed counter,
//     e.g. "✛ Hyperspacing… (20s · still thinking…)". This catches sub-agent /
//     high-effort states that omit "esc to interrupt".
// Neither matches the AskUserQuestion picker ("esc to cancel") nor the idle
// "Brewed for 8h" summary (no "…(Ns").
//
// IMPORTANT: these are only tested against the LAST THINKING_SCAN_LINES lines of
// the capture. _pollThinking captures 26 lines (visible + scrollback history) so
// that parsePanePrompt can find question pickers. Scanning the full 26 lines for
// thinking signals causes false positives: a completed-turn working line that has
// scrolled into history (but is still within the 26-line window) keeps matching
// after generation ends. Limiting to the bottom 8 lines covers the entire visible
// Claude TUI status area while excluding stale scrollback content.
const THINKING_RE = /esc to interrupt/i;
const WORKING_TIMER_RE = /…\s*\(\s*\d+\s*[smh]\b/;
const THINKING_SCAN_LINES = 8;
// Auto/manual compaction renders a distinct working line ("Compacting
// conversation…"). It can run for many seconds with no other output, so without
// a dedicated signal the UI looks hung. Treated as a busy sub-state of thinking.
const COMPACTING_RE = /compacting\b/i;
// A fatal/recoverable API error printed by the CLI when a turn fails (rate
// limit, overload, 5xx). The agent STALLS — it isn't "working" — so the cockpit
// surfaces an [ERROR] badge + a Retry. Matched on a wider window than the
// thinking scan because the error line can sit a few rows above the input box.
const ERROR_SCAN_LINES = 14;
const API_ERROR_RE = /\bAPI Error\b|\boverloaded_error\b|\brate limit(?:ed)?\b[^\n]*\b(?:try again|retry|later)\b|Please try again later/i;

/**
 * @param {string} capture  raw `tmux capture-pane -p` output (ANSI ok)
 * @returns {{ ctxPct: number|null, model: string|null, effort: string|null, thinking: boolean, compacting: boolean, errored: boolean }}
 */
export function parseTuiStatus(capture) {
  const text = String(capture || '').replace(ANSI_RE, '');

  let ctxPct = null;
  const ctxMatch = text.match(CTX_RE);
  if (ctxMatch) {
    const n = Number(ctxMatch[1]);
    if (Number.isFinite(n) && n >= 0 && n <= 100) ctxPct = n;
  }

  let model = null;
  const modelMatch = text.match(MODEL_RE);
  if (modelMatch) model = modelMatch[0].replace(/\s+/g, ' ').trim();

  let effort = null;
  const effortMatch = text.match(EFFORT_RE);
  if (effortMatch) effort = effortMatch[1].toLowerCase();

  // Restrict the thinking-signal scan to the bottom THINKING_SCAN_LINES lines so
  // that stale working/timer lines in the scrollback history (above the visible
  // area) do not produce a false positive after generation ends.
  const lines = text.split('\n');
  const tail = lines.slice(-THINKING_SCAN_LINES).join('\n');
  const compacting = COMPACTING_RE.test(tail);
  // Compaction IS a busy state — fold it into thinking so the rail still reads
  // "working" even if the compaction line omits "esc to interrupt".
  const thinking = compacting || THINKING_RE.test(tail) || WORKING_TIMER_RE.test(tail);

  // API error → only when the agent is NOT actively working (a real stall, not
  // an error string scrolling by mid-generation). Scans a slightly wider window.
  const errTail = lines.slice(-ERROR_SCAN_LINES).join('\n');
  const errored = !thinking && API_ERROR_RE.test(errTail);

  return { ctxPct, model, effort, thinking, compacting, errored };
}

/**
 * Prettify a transcript model id (e.g. "claude-opus-4-8") into a short label
 * ("Opus 4.8"). Falls back to the raw id when the shape is unfamiliar.
 *
 * Handles BOTH id shapes in use:
 *   - dotted    "claude-opus-4-8"  -> "Opus 4.8"
 *   - undotted  "claude-opus-5"    -> "Opus 5"
 * and the full Claude 5 family, `fable` included. The old pattern listed only
 * opus/sonnet/haiku AND required the dotted `-N-N` form, so every Claude 5 id
 * (claude-fable-5, claude-opus-5, claude-sonnet-5) fell through and surfaced as
 * a RAW id in the rail. That was not merely cosmetic: sessions.js resolves the
 * context window by inspecting this label, so an unparsed id also lost its
 * window and mis-reported ctx%.
 *
 * @param {string|null} modelId
 * @returns {string|null}
 */
export function prettyModel(modelId) {
  if (!modelId || typeof modelId !== 'string') return null;
  const m = modelId.match(/(opus|sonnet|haiku|fable)-(\d+)(?:-(\d+))?/i);
  if (m) {
    const family = m[1][0].toUpperCase() + m[1].slice(1).toLowerCase();
    return m[3] ? `${family} ${m[2]}.${m[3]}` : `${family} ${m[2]}`;
  }
  return modelId;
}

/**
 * Resolve the display model label from the two sources the rail has
 * available: a freshly-scraped TUI label (`scraped`, from parseTuiStatus —
 * may carry decoration like "(1M context)" that nothing else knows) and a
 * fallback label (`fallback` — either the transcript-derived label via
 * prettyModel(), or in the incremental poller, the last-known-good label
 * already on the session row).
 *
 * The TUI label normally wins outright — it's the only source that can
 * report the 1M-context decoration. But a narrow or wrapped tmux pane can
 * clip the status line mid-token before MODEL_RE stops matching, producing
 * a half-parsed label:
 *   - "Opus 4.…" clips to "Opus 4." — MODEL_RE's [\d.]+ swallows the dot but
 *     not the "…" that follows, so the match ends on a bare trailing dot. No
 *     real Claude Code status line ever renders a model ending in ".".
 *   - a clip landing before the dot instead produces e.g. "Opus 4" — not
 *     invalid on its own (some families render with no minor version, e.g.
 *     "Opus 5"), but if it is a strict prefix of a KNOWN-good fuller label
 *     ("Opus 4.8"), it's the same clip, one character earlier.
 *
 * Either shape is detected here and the fallback is preferred instead, so a
 * clipped/wrapped capture never surfaces a truncated model in the UI.
 *
 * @param {string|null} scraped   TUI-parsed label for this poll, or null
 * @param {string|null} fallback  transcript-derived or last-known-good label
 * @returns {string|null}
 */
export function resolveModelLabel(scraped, fallback) {
  if (!scraped) return fallback || null;
  const looksTruncated =
    /\.$/.test(scraped) ||
    (!!fallback && fallback !== scraped && fallback.startsWith(scraped));
  return looksTruncated ? (fallback || null) : scraped;
}
