    ...
    ws.on('message', (raw, isBinary) => {
      if (isBinary) {
        ...
        if (entryRef.kind === 'agent') {
          ...
          entryRef.sendTail = (entryRef.sendTail || Promise.resolve())
            .then(() => _sendLiteral(entryRef.target, payload.toString('utf8')))
            .catch((err) => {
              _log({ event: 'pty-agent-send-failed', target: entryRef.target, error: String(err?.message || err) });
            });
          return;
        }
