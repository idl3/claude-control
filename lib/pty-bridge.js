 ...
 * attached client is seeded with one `capture-pane -e` snapshot (`sendAgentSeed`)
...
export function createPtyBridge(deps = {}) {
  ...
  /**
   * One-shot redraw seed for a freshly-attached agent-kind client: a full
   ...
   * @returns {Promise<void>}
   */
  async function sendAgentSeed(entry, ws) {
    try {
      ...
      _log({ event: 'pty-agent-seed-failed', target: entry.target, error: String(err?.message || err) });
    }
  }
