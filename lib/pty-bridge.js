 ...
 * for the exact commands. The ephemeral session shadows the operator's global
 * `after-select-window` hooks so plugins cannot rename shared windows or paint
 * hook failures into this app-owned PTY client. The base session is NEVER
 * killed and its hook configuration is never changed. Because the base session
 * might not exist at all (or its
 ...
 * session is created.
 *
 * AGENT-MODE MIRROR (`kind: 'agent'`, raw pipe-pane passthrough, NOT a `tmux
 * attach`): the Cmd+J full-screen agent overlay needs the OPPOSITE property
 * from the clean single-pane view above — it must NEVER contribute to the
 * live agent pane's window-size negotiation (a phone-sized viewer attaching
 * would shrink the real TUI for every other client on that base session).
 * So agent-kind entries never spawn node-pty and never touch the ephemeral-
 * session machinery at all: `setupAgentEntry` instead creates a FIFO, opens
 * it for reading (`_createReadStream`, kept open for the entry's whole
 * lifetime and fanned out to `entry.clients` exactly like the attach path's
 * `pty.onData`), and starts `tmux.pipePane(target, 'cat >> ' + fifo)`
 * (`pipe-pane -O` — output-only, tmux still owns the pane). A freshly
 * attached client is seeded with one `capture-pane -e` snapshot (`sendPaneSeed`)
 * before joining `entry.clients`, so it sees the live screen immediately
 * instead of a blank canvas until the next delta. Input is the mirror image:
 * `tmux.sendLiteral` (`send-keys -l`, byte-faithful — verified against real
 * tmux 3.6a) instead of `pty.write`. Resize is a client-bookkeeping no-op —
 * an agent pane's size is never driven by a viewer. The shared `has-session`
 * liveness check still runs first for both kinds; see `ensurePty`.
 *
...
import {
  isValidTarget,
  resolveTmuxBin as defaultResolveTmuxBin,
  getSocketPath as defaultGetSocketPath,
  pipePane,
  pipePaneOff,
  sendLiteral,
  capturePane,
...
// How often an agent-kind entry re-queries its live tmux pane's real
// geometry (the pane can change size after attach — its owner resizes their
// terminal, or another client on the shared base session changes the
// smallest-client size) and broadcasts a fresh `pane-size` frame on change.
// See lib/protocol/pty.js's PtyPaneSizeSchema doc comment for why the
// client needs this at all.
const DEFAULT_PANE_SIZE_POLL_MS = Number(process.env.CLAUDE_CONTROL_PTY_PANE_SIZE_POLL_MS) || 2000;
 ...
 *   readStream: import('node:fs').ReadStream|null,
 *   sendTail: Promise<void>|null,
 *   paneCols: number,
 *   paneRows: number,
 *   paneSizeTimer: unknown|null,
 * }} PtyEntry
 */
...
/** Re-exported for compatibility; the owner lives in tmux.js so pane discovery
 * can prefer a real session over its PTY mirror before deduplication. */
export { EPHEMERAL_SESSION_PREFIX } from './tmux.js';
...
/**
 * Default FIFO creator for agent-kind entries: shells out to the `mkfifo`
 * binary (argv array, never a shell) — Node has no built-in FIFO-creation
 * syscall wrapper. Hermetic tests always inject their own `mkfifo` stub; see
 * test/pty-bridge.test.js.
 * @param {string} fifoPath
 * @returns {Promise<void>}
 */
async function defaultMkfifo(fifoPath) {
  await execFileP('mkfifo', [fifoPath]);
}

/**
 * Default agent-kind FIFO path: unique per attach, under the OS temp dir,
 * namespaced with a sanitized copy of the target so a stray leftover fifo
 * (e.g. after an unclean process exit) is traceable back to its session.
 * @param {string} target
 * @returns {string}
 */
function defaultMakeFifoPath(target) {
  const sanitized = String(target).replace(/[^A-Za-z0-9_-]/g, '_').slice(0, 100);
  return path.join(os.tmpdir(), `ccpty-agent-${sanitized}-${process.pid}-${randomUUID()}`);
}

/**
 * Default FIFO cleanup — a plain unlink that does NOT swallow errors itself;
 * `teardownAgentEntry` is the non-throwing boundary (mirrors
 * `killEphemeralSession`'s own try/catch-and-log pattern).
 * @param {string} fifoPath
 * @returns {Promise<void>}
 */
async function defaultUnlinkFifo(fifoPath) {
 ...
 *   resolveTarget?: (sessionId: string) => Promise<{target: string, mode?: string}|null>|{target: string, mode?: string}|null,
 ...
 *   pipePane?: (target: string, shellCommand: string) => Promise<void>,
 *   pipePaneOff?: (target: string) => Promise<void>,
 *   sendLiteral?: (target: string, text: string) => Promise<void>,
 *   capturePane?: typeof capturePane,
 *   cursorPosition?: (target: string) => Promise<{x: number, y: number}>,
 *   mkfifo?: (fifoPath: string) => Promise<void>,
 *   createReadStream?: (fifoPath: string) => import('node:fs').ReadStream,
 *   unlinkFifo?: (fifoPath: string) => Promise<void>,
 *   makeFifoPath?: (target: string) => string,
 *   paneSize?: (target: string) => Promise<{cols: number, rows: number}>,
 *   schedulePaneSizePoll?: (fn: () => void, ms: number) => unknown,
 *   clearPaneSizePoll?: (timer: unknown) => void,
 *   paneSizePollMs?: number,
 * }} [deps]
 */
export function createPtyBridge(deps = {}) {
  ...
  // Agent-kind (kind: 'agent') seams — see the module header's "AGENT-MODE
  // MIRROR" section. All default to the real lib/tmux.js exports / real fs;
  // hermetic tests inject fakes for every one of these (no tmux, no
  // filesystem access).
  const _pipePane = deps.pipePane || pipePane;
  const _pipePaneOff = deps.pipePaneOff || pipePaneOff;
  const _sendLiteral = deps.sendLiteral || sendLiteral;
  const _capturePane = deps.capturePane || capturePane;
  const _cursorPosition = deps.cursorPosition || cursorPosition;
  const _mkfifo = deps.mkfifo || defaultMkfifo;
  const _createReadStream = deps.createReadStream || fs.createReadStream;
  const _unlinkFifo = deps.unlinkFifo || defaultUnlinkFifo;
  const _makeFifoPath = deps.makeFifoPath || defaultMakeFifoPath;
  const _paneSize = deps.paneSize || paneSize;
  const _schedulePaneSizePoll = deps.schedulePaneSizePoll || ((fn, ms) => {
    const t = setInterval(fn, ms);
    t.unref?.();
    return t;
  });
  const _clearPaneSizePoll = deps.clearPaneSizePoll || clearInterval;
  const _paneSizePollMs = deps.paneSizePollMs ?? DEFAULT_PANE_SIZE_POLL_MS;
  ...
  /**
   * Best-effort, non-throwing teardown of an agent-kind entry's FIFO stream:
   * stop `pipe-pane` (so the tmux-side `cat` has nothing left to write),
   * THEN destroy our read stream, THEN unlink the FIFO file. That order
   * matters — destroying the reader first would leave `cat`'s next write
   * hitting a reader-less FIFO (EPIPE) while pipe-pane is still nominally
   * active, which is a noisier failure than simply asking tmux to stop
   * first. Every step logs-and-continues rather than throwing, mirroring
   * `killEphemeralSession`'s pattern for the attach-kind teardown path.
   * @param {PtyEntry} entry
   * @returns {Promise<void>}
   */
  async function teardownAgentEntry(entry) {
    if (entry.paneSizeTimer) {
      _clearPaneSizePoll(entry.paneSizeTimer);
      entry.paneSizeTimer = null;
    }
    try {
      await _pipePaneOff(entry.target);
    } catch (err) {
      _log({ event: 'pty-agent-pipe-off-failed', target: entry.target, error: String(err?.message || err) });
    }
    if (entry.readStream) {
      const stream = entry.readStream;
      entry.readStream = null;
      try { stream.destroy(); } catch { /* already destroyed */ }
    }
    if (entry.fifoPath) {
      const fifoPath = entry.fifoPath;
      entry.fifoPath = null;
      try {
        await _unlinkFifo(fifoPath);
      } catch (err) {
        _log({ event: 'pty-agent-fifo-unlink-failed', fifoPath, error: String(err?.message || err) });
      }
    }
  }

  /**
   * Agent-kind setup (kind === 'agent'): no ephemeral session, no node-pty
   * spawn. Creates a FIFO, opens its read end (kept open + draining for the
   * entry's whole lifetime, fanned out to `entry.clients` exactly like the
   * attach path's `pty.onData`), then starts `pipe-pane -O` writing into it.
   * See the module header's "AGENT-MODE MIRROR" section for the full
   * rationale (why no `tmux attach` is ever used here).
   *
   * Order matters: the read stream is created (which begins its `open()`
   * asynchronously) BEFORE `pipePane` starts the tmux-side `cat >> fifo`
   * writer — so the writer's own `open()` rendezvous with an already-in-
   * flight reader instead of blocking on a fifo nobody has begun to open.
   *
   * @param {string} sessionId
   * @param {PtyEntry} entry
   * @returns {Promise<void>}
   */
  async function setupAgentEntry(sessionId, entry) {
    const fifoPath = _makeFifoPath(entry.target);
    await _mkfifo(fifoPath);
    entry.fifoPath = fifoPath;

    const stream = _createReadStream(fifoPath);
    ...
    stream.on('error', (err) => {
      _log({ event: 'pty-agent-fifo-read-error', target: entry.target, error: String(err?.message || err) });
    });
    // The piped `cat` process is killed when tmux tears down the pane/session
    // (or when pipePaneOff runs during OUR OWN teardown below) — either way
    // its stdout (this fifo's write end) closes and the read stream sees
    // EOF. An EOF that happens OUTSIDE our own teardown (entry.alive still
    // true) is a genuine session-ended event, unified with the attach-kind
    // dead-target framing (close 4000) below.
    stream.on('end', () => {
      if (!entry.alive) return; // already tearing down via reap/evictOne
      entry.alive = false;
      if (entry.idleTimer) _clearIdleReap(entry.idleTimer);
      ptys.delete(sessionId);
      teardownAgentEntry(entry);
      killEntryClientsWithError(entry, 'dead-target', 'tmux agent pane ended');
    });

    await _pipePane(entry.target, `cat >> ${fifoPath}`);
    entry.alive = true;

    // Best-effort initial pane geometry — the client sizes ITS OWN xterm grid
    // to match this exactly, then visually scales the result to fill the
    // viewport (see PtyPaneSizeSchema's doc comment in lib/protocol/pty.js).
    // A query failure just means the client falls back to its own default
    // grid; not fatal to the attach.
    try {
      const size = await _paneSize(entry.target);
      entry.paneCols = size.cols;
      entry.paneRows = size.rows;
    } catch (err) {
      _log({ event: 'pty-agent-pane-size-query-failed', target: entry.target, error: String(err?.message || err) });
    }

    // Live-resize watch: the real pane can change size after attach (its
    // owner resizes their terminal, or another client on the shared base
    // session changes the smallest-client size). Poll and broadcast a fresh
    // `pane-size` frame to every currently-attached client on change.
    entry.paneSizeTimer = _schedulePaneSizePoll(async () => {
      if (!entry.alive) return;
      try {
        const size = await _paneSize(entry.target);
        if (size.cols === entry.paneCols && size.rows === entry.paneRows) return;
        entry.paneCols = size.cols;
        entry.paneRows = size.rows;
        for (const ws of entry.clients.keys()) {
          sendControl(ws, { type: 'pane-size', paneCols: size.cols, paneRows: size.rows });
        }
      } catch (err) {
        _log({ event: 'pty-agent-pane-size-poll-failed', target: entry.target, error: String(err?.message || err) });
      }
    }, _paneSizePollMs);
  }

  /**
 * One-shot redraw seed for a freshly-attached client: a full
 * screen clear + the pane's current visible contents (SGR-preserving,
   * `capture-pane -e`, visible screen only — never scrollback, which would
   * misrepresent what's currently on screen) + a cursor reposition, sent as
   * a single 0x00-framed binary frame directly to `ws` — NOT broadcast via
   * `entry.clients`, since this client isn't registered there yet.
   *
   * Ordering (deliberate): called AFTER the `attached` control frame and
   * BEFORE `ws` is added to `entry.clients`. A live delta arriving in that
   * sub-ms gap is simply painted over by this seed's own snapshot (taken
   * after the delta), so nothing is lost — self-healing. The reverse order
   * (register first, seed after) would risk a genuinely newer delta being
   * overwritten by a stale seed, which does NOT self-heal.
   *
   * Best-effort: a failure here logs and returns without throwing — the
   * client still attaches successfully and simply misses the initial
   * redraw (the next live delta repaints normally).
   *
   * @param {PtyEntry} entry
   * @param {{ send?: Function }} ws
 * @param {{ skipEmpty?: boolean, errorEvent?: string }} [opts]
 * @returns {Promise<void>}
 */
  async function sendPaneSeed(entry, ws, { skipEmpty = false, errorEvent = 'pty-pane-seed-failed' } = {}) {
    try {
      const [text, cursor] = await Promise.all([
        _capturePane(entry.target, 0, true, false, { visibleOnly: true }),
        _cursorPosition(entry.target),
      ]);
      // Attach-kind hermetic tests (and a genuinely blank shell) can produce
      // an empty pane. Do not manufacture a clear-screen frame for that path;
      // live PTY output remains authoritative. Agent-pane callers keep the
      // historical empty-screen redraw by leaving skipEmpty=false.
      if (skipEmpty && text.length === 0) return;
      const body = text.split('\n').join('\r\n');
      const redraw = `\x1b[2J\x1b[H${body}\x1b[${cursor.y + 1};${cursor.x + 1}H`;
      const framed = Buffer.concat([PTY_DATA_HEADER, Buffer.from(redraw, 'utf8')]);
      sendWsMessage(ws, framed, { limitBytes: _wsBufferLimitBytes });
    } catch (err) {
      _log({ event: errorEvent, target: entry.target, error: String(err?.message || err) });
    }
  }
   ...
   * on exit/reap). Never throws.
   *
   * NOT run at bridge construction. The bridge is built while server.js is
   * still being imported — before it knows whether it can bind its port — so
   * a DOOMED duplicate boot (`npm start` while the service is up: reaps
   * nothing, then exits EADDRINUSE) used to sweep first and kill the LIVE
   * instance's in-use ephemeral sessions on its way out. Every open terminal
   * then saw its `tmux attach` exit, i.e. a dead-target close-4000
   * ("session ended") with no auto-reconnect. Under launchd's KeepAlive that
   * doomed boot repeats every few seconds, so terminals broke continuously.
   * server.js now calls this ONLY after `listen` succeeds — owning the port
   * is what makes "every other `_ccpty_*` is an orphan of mine" true.
   *
   * Second guard, for instances that legitimately coexist on DIFFERENT ports
   * (a hermetic test boot, a dev instance): an ephemeral session that still
   * has a client attached belongs to a LIVE attach, so it is never an orphan.
   * A crashed owner's node-pty client is gone, so its leftovers read
   * attached=0 and are still swept. Residual gap: a session created by
   * `new-session -d` microseconds before its `tmux attach` lands reads
   * attached=0 — tiny, and now only reachable during another instance's boot.
   *
   * @returns {Promise<void>}
   */
  async function sweepOrphans() {
    try {
      ...
      const { stdout } = await _runTmuxCmd(tmuxBin, [
        '-S', socket, 'list-sessions', '-F', '#{session_name}\t#{session_attached}',
      ]);
      const orphans = [];
      for (const line of String(stdout || '').split('\n')) {
        const [name, attached] = line.trim().split('\t');
        if (!name || !name.startsWith(EPHEMERAL_SESSION_PREFIX)) continue;
        if (Number(attached) > 0) {
          _log({ event: 'pty-ephemeral-orphan-sweep-skipped-attached', name });
          continue; // a live attach owns this one
        }
        orphans.push(name);
      }
   ...
   * @param {'attach'|'agent'} [mode='attach']
   * @returns {Promise<PtyEntry>}
   * @throws {DeadTargetError}
   */
  async function ensurePty(sessionId, target, mode = 'attach') {
    ...
    const kind = mode === 'agent' ? 'agent' : 'attach';
    ...
    entry.ready = (async () => {
      ...
      // shared by BOTH kinds. `new-session -t <base>` on a non-existent base
      // does NOT reliably fail (see module header) so it can't be trusted as
      // the dead-target signal on its own — `has-session` validates
      // precisely and fails loudly, and crucially runs BEFORE anything else
      // (ephemeral session, fifo, pipe-pane) is created.
      const tmuxBin = await _resolveTmuxBin();
      const socket = await _getSocketPath();
      ...
      if (kind === 'agent') {
        // Agent-kind mirror: no ephemeral session, no node-pty attach spawn —
        // see the module header's "AGENT-MODE MIRROR" section. Any failure
        // here is normalized to a DeadTargetError so the attach-handler's
        // catch (below, in handleConnection) frames it identically to every
        // other attach failure.
        try {
          await setupAgentEntry(sessionId, entry);
        if (entry.aborted) {
          ptys.delete(sessionId);
          await teardownAgentEntry(entry);
          throw new Error('pty setup aborted mid-flight');
        }
        } catch (err) {
          ptys.delete(sessionId);
          await teardownAgentEntry(entry);
          throw err instanceof DeadTargetError
            ? err
            : new DeadTargetError(
                `failed to start agent-pane stream for target ${JSON.stringify(target)}: ${err?.message || err}`,
              );
        }
        return;
      }
      ...
      try {
        ...
        // This app-owned mirror session must not run the operator's global
        // after-select-window hooks. Plugins commonly rename windows from that
        // hook; grouped sessions share those windows with the real base session,
        // and a failing background hook can also paint its error into this PTY
        // client after our CodeWhale capture seed. A session-local empty hook
        // shadows the global array only for `_ccpty_*` — the base session and
        // the user's tmux configuration remain untouched.
        await _runTmuxCmd(tmuxBin, ['-S', socket, 'set-hook', '-t', ephemeralName, 'after-select-window', '']);
      ...
      // Drain attach-time output BEFORE the dead-target grace wait. tmux paints
      // its initial client screen (and may print plugin-hook diagnostics)
      // immediately after spawn. Registering this listener after the wait lets
      // node-pty deliver those buffered bytes only after the later capture-pane
      // seed, overwriting the real pane with stale attach noise. No clients are
      // registered yet, so output during the grace window is intentionally
      // consumed; every client receives an authoritative pane seed below.
      handle.onData((chunk) => {
  ...
  function resizeClient(sessionId, ws, cols, rows) {
    ...
    entry.lastUsed = _now();
    // Agent-kind entries are structurally decoupled from client sizing (the
    // whole point of the fifo/pipe-pane mirror over a shared `tmux attach` —
    // see the module header): there is no pty to resize and no client-size
    // negotiation to perform. Track the client's reported size (above, for
    // bookkeeping/diagnostics parity with attach-kind) and stop.
  ...
  function reap(sessionId) {
    ...
    ptys.delete(sessionId);
    if (entry.kind === 'agent') {
      teardownAgentEntry(entry); // fire-and-forget — never throws
      return;
    }
    if (entry.pty) entry.pty.kill();
  ...
  function handleConnection(ws, req) {
    ...
    ws.on('message', (raw, isBinary) => {
      if (isBinary) {
        if (!sessionId || !entryRef) return;
        const buf = Buffer.isBuffer(raw) ? raw : Buffer.from(raw);
        if (buf.length < 1 || buf[0] !== PTY_CHANNEL_DATA) return;
        const payload = buf.subarray(1);
        if (entryRef.kind === 'agent') {
          // No pty to write to — literal bytes go straight to the live tmux
          // pane via `send-keys -l` (see module header). Chained on the
          // ENTRY's `sendTail` (not a per-connection/closure variable) so
          // concurrent input frames — fast typing or a multi-frame paste,
          // possibly from multiple viewers of the same pane — deliver in
          // strict FIFO order instead of racing as concurrent `tmux
          // send-keys` execs whose OS completion order is nondeterministic
          // (which would reorder characters at the agent pane). A failed
          // send is logged but does NOT break the chain — `.catch` resolves
          // so later keystrokes still flow.
          // Capture the entry + target NOW: `entryRef` is nulled on detach, and
          // these callbacks run a tick later — reading `entryRef.target` inside
          // them threw `Cannot read properties of null` as an unhandled
          // rejection whenever a keystroke raced the client going away.
          const entry = entryRef;
          const sendTarget = entry.target;
          entry.sendTail = (entry.sendTail || Promise.resolve())
            .then(() => _sendLiteral(sendTarget, payload.toString('utf8')))
            .catch((err) => {
              _log({ event: 'pty-agent-send-failed', target: sendTarget, error: String(err?.message || err) });
            });
          return;
        }
        if (!entryRef.pty) return;
        try { entryRef.pty.write(payload); } catch { /* pty may have just exited */ }
        return;
      }
      ...
      if (m.type === 'attach') {
        ...
        (async () => {
          try {
            ...
            const mode = resolved && resolved.mode === 'agent-pane' ? 'agent' : 'attach';
            const entry = await ensurePty(requestedId, target, mode);
            ...
            if (entry.kind === 'agent') {
              // Seed BEFORE registering `ws` in entry.clients: a live delta
              // arriving in this sub-ms gap is simply painted over by the
              // seed's own (later) snapshot — self-healing. The reverse
              // order risks a genuinely newer delta being overwritten by a
              // stale seed, which does NOT self-heal. See sendAgentSeed's
              // own doc comment for the full rationale.
              sendControl(ws, { type: 'attached', sessionId: requestedId });
              // Pane geometry BEFORE the seed: the client must size its own
              // xterm grid to match the real pane (term.resize, synchronous)
              // before the seed's capture-pane text is written into it —
              // otherwise the seed paints onto whatever default-sized grid
              // the client started with. Guarded on a successful earlier
              // query (0x0 means setupAgentEntry's own query failed).
              if (entry.paneCols > 0 && entry.paneRows > 0) {
                sendControl(ws, { type: 'pane-size', paneCols: entry.paneCols, paneRows: entry.paneRows });
              }
              await sendPaneSeed(entry, ws);
              entry.clients.set(ws, { cols: 0, rows: 0 });
            } else {
              sendControl(ws, { type: 'attached', sessionId: requestedId });
              // `tmux attach` paints its initial screen immediately, during
              // ensurePty's dead-target grace window, before the bridge begins
              // forwarding node-pty output. Seed every viewer from the live
              // pane so the terminal never stays black until a future redraw.
              // Register after the snapshot: a delta during capture is folded
              // into that later snapshot instead of being overwritten by it.
              await sendPaneSeed(entry, ws, {
                skipEmpty: true,
                errorEvent: 'pty-attach-seed-failed',
              });
              entry.clients.set(ws, { cols: 0, rows: 0 });
            }
          } catch (err) {
  ...
  return { handleConnection, shutdownAll, liveCount, sweepOrphans };
}
