...
function _rememberCacheEntry(cache, key, value, maxKeys = CACHE_MAX_KEYS) {
...
function _pruneExpiredCache(cache, now, ttlMs = CACHE_TTL_MS) {
...
export function listSkills(cwd) {
  ...
  if (hit && now - hit.ts < CACHE_TTL_MS) {
    _rememberCacheEntry(_cache, cacheKey, hit);
    return hit.skills;
  }
  _pruneExpiredCache(_cache, now);
  ...
  _rememberCacheEntry(_cache, cacheKey, { skills, ts: now });
  return skills;
}
...
export function _cacheSizeForTest() {
  return _cache.size;
}
