 ...
 * spawn↔completion accumulator. Only two record shapes matter:
 *
 ...
 *    `<task-notification>...<status>...</status>...</task-notification>`:
 *    ASYNC completion (authoritative done) for both the agentId and the
 *    toolUseId it carries.
 *
 * Everything else (tool_use spawn blocks, assistant records, etc.) is
 * ignored — the roster of "who might be running" comes from the subagents
 * dir listing, not from parsing spawn blocks here.
 *
 * @param {string} text  newly-read transcript bytes (one or more JSONL lines)
 * @param {{completedToolUseIds:Set<string>, completedAgentIds:Set<string>, asyncLaunchedToolUseIds:Set<string>}} acc
 * @returns {typeof acc}
 */
 ...
 *
 * @param {{kind: 'claude'|'codex'|string, transcriptPath: string|null}} args
...
export function computeSubAgentActivity({ kind, transcriptPath }) {
  ...
  if (kind === 'claude') {
    ...
    for (const name of entries) {
      ...
      if (now - mtimeMs >= FALLBACK_IDLE_MS) continue; // quiet beyond fallback → done

      // agentId-keyed completion (async <task-notification>) needs no meta read.
      if (acc.completedAgentIds.has(agentId)) continue;

      // Only recent, not-yet-agentId-completed agents pay the meta read — needed
      // solely to resolve the sync tool_result completion (keyed by toolUseId).
      let toolUseId = null;
      try {
...
export class SubAgentsWatcher extends EventEmitter {
  ...
  _refreshCanonical() {
    ...
    for (const agent of this._agents.values()) {
      const toolUseDone = !!(agent.toolUseId && this._parentAcc.completedToolUseIds.has(agent.toolUseId));
      const agentIdDone = this._parentAcc.completedAgentIds.has(agent.agentId);
      agent.canonicalDone = toolUseDone || agentIdDone;
  ...
  /**
   * Status: canonical-first. A scanned spawn↔completion signal from the parent
   * transcript (sync tool_result, or async <task-notification>) is authoritative
   * and instant. Absent that, fall back to transcript freshness — a live
   * sub-agent's file is actively appended; a genuinely finished one goes static.
   * The freshness fallback is intentionally generous (FALLBACK_IDLE_MS) so a
   * running-but-quiet agent (inference pause, long tool call) never flickers to
   * "done" just because its file went quiet for a while.
   */
  _statusFor(a) {
    if (a.canonicalDone) return 'done'; // scan-authoritative, instant
    ...
    // doneByParent) while the agent keeps writing for minutes — without this
    // override it would wrongly read as done the whole run.
    if (age < ACTIVE_WINDOW_MS) return 'running';
    // Quiet file, no canonical signal yet: doneByParent is a legacy block-based
    // hint — only trust it when it's NOT just an async launch-ack (that only
    // means "spawned", not "finished"; the real completion is the later
    // <task-notification> handled by canonicalDone above).
    if (a.doneByParent && !a.asyncLaunched) return 'done';
    // Generous mtime fallback: covers agents that predate the scan window,
    // whose meta/toolUseId hasn't arrived yet, or nested (depth>=2) agents.
