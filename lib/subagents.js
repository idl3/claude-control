 ...
 *
 * @param {{kind: 'claude'|'codex'|'claudex'|'claudemi'|string, transcriptPath: string|null}} args
...
export function computeSubAgentActivity({ kind, transcriptPath }) {
  ...
  // claudex/claudemi write an ordinary claude-format transcript (same binary,
  // only the upstream ANTHROPIC_BASE_URL differs) — the claude-shaped scan
  // below applies unchanged. The cache key stays prefixed 'claude:' regardless
  // (scoped by the session's own unique transcriptPath, so a claude, claudex,
  // and claudemi session never collide).
  if (kind === 'claude' || kind === 'claudex' || kind === 'claudemi') {
    const acc = _scanIncremental(
