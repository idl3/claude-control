...
export class SubAgentsWatcher extends EventEmitter {
  ...
  async load(agentId) {
    ...
    agent.loadPromise = this._loadSnapshot(agent).finally(() => {
  ...
  _mtime(agent) {
    try { return fs.statSync(agent.jsonlPath).mtimeMs; } catch { return 0; }
  }
