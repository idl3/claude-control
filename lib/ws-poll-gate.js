 ...
 */
export function createWsPollGate(registry, resources) {
  ...
  // registry.start() has no re-entrancy guard, so calling it while already
  // running would leak a second set of setIntervals rather than being a
  // harmless no-op.
  let paused = false;
