...
const DEFAULT_OVERLIMIT_COOLDOWN_MS = 60_000;
...
async function reclaimableFreeBytes() {
  ...
  try {
    const { stdout } = await execFileAsync('vm_stat', [], { encoding: 'utf8', timeout: 1000, maxBuffer: 64 * 1024 });
...
async function powerStatus() {
  if (process.platform !== 'darwin') return null;
  try {
    const { stdout } = await execFileAsync('pmset', ['-g', 'batt'], { encoding: 'utf8', timeout: 1000, maxBuffer: 64 * 1024 });
...
export class ResourceMonitor extends EventEmitter {
  /**
   * @param {{ intervalMs?: number, rssLimitMB?: number, overlimitCooldownMs?: number }} opts
   */
  constructor({
    intervalMs = 5000,
    rssLimitMB = 350,
    overlimitCooldownMs = DEFAULT_OVERLIMIT_COOLDOWN_MS,
  } = {}) {
    super();
    this._intervalMs = intervalMs;
    this._rssLimitMB = rssLimitMB;
    this._overlimitCooldownMs = Number.isFinite(overlimitCooldownMs) && overlimitCooldownMs >= 0
      ? overlimitCooldownMs
      : DEFAULT_OVERLIMIT_COOLDOWN_MS;
    this._timer = null;
    this._overLimit = false;
    this._lastOverlimitAt = 0;
    ...
    // every 5th tick (~25s). Cache between refreshes.
  ...
  /**
   * Force one sample outside the timer cadence (R8: server.js calls this on
   * WS reconnect after a zero-client pause, so the first post-resume
   * broadcast isn't stale until the next interval). Delegates to the same
   * _tick() the timer calls, so the existing _ticking re-entrancy guard
   * still applies if a real tick is already in flight.
   *
  ...
  /**
   * Emit immediately on a rising edge, then periodically while pressure stays
   * high. A single trim may not bring RSS below the limit, so rising-edge-only
   * signaling can otherwise prevent any further relief for the life of the
   * process.
   *
   * @param {{ overLimit: boolean }} snap
   * @param {number} now
   */
  _maybeEmitOverlimit(snap, now = Date.now()) {
    if (!snap.overLimit) {
      this._overLimit = false;
      this._lastOverlimitAt = 0;
      return;
    ...
    const cooldownElapsed = now - this._lastOverlimitAt >= this._overlimitCooldownMs;
    if (!this._overLimit || cooldownElapsed) {
      this._overLimit = true;
      this._lastOverlimitAt = now;
