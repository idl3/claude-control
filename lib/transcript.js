// lib/transcript.js — bounded transcript tailing for claude-control.
...
// Env lookup mirroring server.js: CLAUDE_CONTROL_<X> only. The pre-rename
...
// Override with CLAUDE_CONTROL_TAIL_BYTES.
...
// Override with CLAUDE_CONTROL_MAX_BUFFER.
...
export class TranscriptTailer extends EventEmitter {
   ...
   */
  _trackPending(msg) {
    // A question is pending only while it is the transcript's FINAL turn. Any
    // `user` message ends the wait — whether the human SELECTED an option (a
    // tool_result, which is carried on a user-role message) or TYPED a free-text
    // reply (which Claude Code records with NO tool_result for the question).
    // Clearing on the user turn covers both; the old tool_result-only close left
    // a typed-over question dangling in _pendingMap forever, so the session read
    // as perpetually waiting after it was answered.
    if (msg.role === 'user') {
      this._pendingMap.clear();
      return;
    }
