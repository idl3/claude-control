...
// Default message-count cap for the in-memory buffer. Raised from 500 to 1500
// so that, within the 8 MB tail window, the count cap is what governs how much
// history a fresh subscribe serves — not the byte window silently truncating
// recent user turns. At a few hundred bytes/normalized message this is a few MB
// of resident memory per open session, well within the server's RSS budget.
// Override with CLAUDE_CONTROL_MAX_BUFFER (legacy: COCKPIT_MAX_BUFFER).
const DEFAULT_MAX_BUFFER = envInt('MAX_BUFFER') ?? 1500;
