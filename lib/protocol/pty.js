...
/**
 * Server -> client, agent-kind sessions ONLY: the real tmux pane's current
 * geometry. Agent-mode never resizes the pane to match a viewer (see
 * resizeClient's early-return in pty-bridge.js) — the client uses this to
 * size ITS OWN xterm grid to match the pane exactly, then visually scales
 * the result to fill the viewport, instead of the usual "fit the grid to
 * the container" flow. Sent once on attach (if the pane's geometry could be
 * queried) and again whenever the live pane is resized by its real owner.
 */
export const PtyPaneSizeSchema = z.object({
  type: z.literal('pane-size'),
  paneCols: z.number().int().positive(),
  paneRows: z.number().int().positive(),
});
...
/** @typedef {import('zod').z.infer<typeof PtyPaneSizeSchema>} PtyPaneSize */
