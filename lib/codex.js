...
// ---------------------------------------------------------------------------
// Strip ANSI/OSC escapes so detection works whether the caller captured the pane
// with escapes (-e) or without. Both current callers pass escapes=false, but a
// future -e caller would otherwise silently get null (the escapes break heading
// and option matching). Mirrors lib/prompt.js's stripAnsi.
// eslint-disable-next-line no-control-regex
const CODEX_ANSI_RE = /\x1b\[[0-9;?]*[A-Za-z]|\x1b\][^\x07]*\x07|\x1b[()][AB0]/g;
...
export function detectPendingFromCapture(capture) {
  ...
  const lines = String(capture).replace(CODEX_ANSI_RE, '').split('\n');
...
// `model`, when given, appends `--model <id>` — the top-level `codex` binary
// (unlike `codex app-server`, which has no per-invocation model flag; that
// transport instead sets the model via the RPC `thread/start` params, see
// lib/codex-rpc.js) accepts `-m/--model <MODEL>` per `codex --help`.
// `skipPermissions`, when true, appends `--dangerously-bypass-approvals-and-
// sandbox` (the exact flag `codex --help` documents for this CLI version) so
// a fresh Codex tmux session launches without approval/sandbox prompts, same
// intent as Claude's `--dangerously-skip-permissions` below.
// ---------------------------------------------------------------------------
export function buildSpawnCommand({ cwd, bin = 'codex', model, skipPermissions } = {}) {
  const args = ['-C', cwd];
