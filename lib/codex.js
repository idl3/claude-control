    ...
    await Promise.all(
      rollouts.map(async (filename) => {
        ...
        try {
          ...
          // Delegate per-file parsing to readRolloutMeta. Note: readRolloutMeta
          // re-stats the file, which is a cheap redundant stat — acceptable.
          const discovered = await readRolloutMeta(filePath, now);
