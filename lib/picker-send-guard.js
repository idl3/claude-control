...
export function shouldRefuseSendForPicker({ viaAnswer, kind, transport, parsedPicker }) {
  ...
  // + pane-scrape menus, via parsePanePrompt), claudex (the same claude binary
  // and TUI, pointed at the olam auth-worker — parsePanePrompt applies unchanged),
  // and Codex (exec/patch/trust approvals + numbered questions, via
  // parseCodexPrompt) all qualify. The caller picks the parser by kind and passes
  // its result as parsedPicker; this predicate only reads the boolean presence,
  // so one decision path covers all three kinds.
  if (kind !== 'claude' && kind !== 'claudex' && kind !== 'codex') return false;

  // claude-print transport is not a keystroke TUI — no tmux picker to guard.
  // (Codex panes never use the 'print' transport; claudex is ALWAYS tmux — never
  // print — per server.js's spawn-time force, so this is a Claude-only carve-out
  // in practice, but the literal check stays kind-agnostic on transport alone.)
