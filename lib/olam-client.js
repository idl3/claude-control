...
export class OlamOrgClient {
  ...
  constructor(orgCfg, deps = {}) {
    ...
    // In-memory only; never logged, never returned.
    this._jwt = null;
    this._runnerToken = null; // { value, label }
  ...
  /** Drop the cached JWT (call on 401 from the SPA; next call re-mints). */
  ...
  async _spaFetch(path) {
    ...
    let res = await this.fetch(`${this.cfg.spaBase}${path}`, {
      headers: { 'cf-access-token': jwt },
    });
    if (res.status === 401 || res.status === 403) {
      // Expired Access session: re-mint once (cloudflared refreshes silently
      // while the org-level login is valid), then surface NoAccessSession.
      this.invalidateAccessToken();
      const fresh = await this.accessToken();
      res = await this.fetch(`${this.cfg.spaBase}${path}`, {
        headers: { 'cf-access-token': fresh },
      });
  ...
  async runnerStatus(sessionId, pool, { retried = false } = {}) {
    ...
    if (res.status === 401 && !retried) {
      this._runnerToken = null; // single re-walk (audit note: rotation suspected)
      return this.runnerStatus(sessionId, pool, { retried: true });
    }
    if (!res.ok) throw new Error(`[${this.org}] runner status HTTP ${res.status} (${sessionId})`);
    return res.json();
  }
   ...
   * confirmed once per session (cached): the runner 200s on every pool, so a
   * non-empty `phase` (or done=true) marks the pool that actually ran it.
   * Sessions with no signal on any pool stay pool=null (list-only row).
   */
  async enrich(sessions, { maxProbes = 10 } = {}) {
    let probes = 0;
    for (const s of sessions) {
      const pools = s.pool ? [s.pool] : POOL_ORDER;
      for (const pool of pools) {
        if (probes >= maxProbes) return sessions;
        probes += 1;
        let status;
        try {
