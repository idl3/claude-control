...
// listSessions() page size. The SPA silently defaults `scope=all` to 50 rows
// with no `limit` param — an org with >50 sessions was truncated with no
// signal. Empirically walked live (2026-07): `limit=200` returns real
// additional rows past 50; `limit=1000`/`10000` both still cap at exactly
// 200 — the backend's own hard ceiling. Request that ceiling explicitly and
// track when we hit it (`this.capped`) so callers can render an honest "N+"
// instead of a possibly-wrong exact count.
...
export class OlamOrgClient {
   ...
   */
  async listSessions() {
    const res = await this._spaFetch(
      `/api/plan-chat/v1/sessions?type=chat&scope=all&limit=${LIST_SESSIONS_LIMIT}`,
    );
    if (!res.ok) throw new Error(`[${this.org}] sessions list HTTP ${res.status}`);
    const body = await res.json();
    const rows = Array.isArray(body) ? body : (body?.sessions ?? []);
    this.capped = rows.length >= LIST_SESSIONS_LIMIT;
