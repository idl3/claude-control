...
// listSessions() now pages via an opaque cursor (backend PR, 2026-07): each
// call requests LIST_SESSIONS_PAGE (50) rows and the server hands back
// `nextCursor` for the next page. LIST_SESSIONS_LIMIT (200) is kept ONLY for
// back-compat with anything still importing it — it is NO LONGER sent as the
// request limit. `this.capped` is now a legacy-only fallback signal: it fires
// ONLY when a server predating cursor pagination returns a full (>=50-row)
// page with NO `nextCursor` field at all (bare array, or `{sessions}` with no
// cursor key) — the "may have more, no way to ask for the rest" lower-bound
// this PR replaces. A migrated server (any response carrying a `nextCursor`
// field, even `null`) never sets `capped`.
export const LIST_SESSIONS_LIMIT = 200;
export const LIST_SESSIONS_PAGE = 50;
...
export class OlamOrgClient {
  ...
  constructor(orgCfg, deps = {}) {
    ...
    Object.defineProperty(this, '_operatorSub', { value: null, writable: true, enumerable: false });
    ...
    this._pools = new Map();
    // true when the last listSessions() hit LIST_SESSIONS_LIMIT exactly —
    // i.e. there may be more rows beyond the page we fetched. Not a secret;
    // enumerable so /api surfaces + the frontend org tab can render "N+".
    this.capped = false;
    // Opaque cursor for the NEXT page (backend PR), null when the last
    // listSessions() call reported no more pages (or hit a pre-migration
    // server with no nextCursor field at all). Not a secret; enumerable like
    // `capped` so /api surfaces + the SPA's scroll-paging can read it directly.
  ...
  /**
   * The operator's CF Access `sub` claim — the stable per-identity id the SPA
   * stores as a session's owner (its `owner_email` field is actually the actor
   * SUB, not an email). Read-only must compare sub-to-sub: an email-vs-sub
   * compare never matches, which falsely marked every session the operator owns
   * as read-only and disabled steer/resume on their own sessions. Returns ''
   * when unavailable. Cached per JWT (cleared by invalidateAccessToken).
   */
  async operatorSub() {
    if (this._operatorSub != null) return this._operatorSub;
    const jwt = await this.accessToken();
    let sub = '';
    try {
      const payload = jwt.split('.')[1];
      const json = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'));
      if (typeof json.sub === 'string') sub = json.sub;
    } catch {
      sub = '';
    }
    this._operatorSub = sub;
    return sub;
  }
  ...
  async listSessions({ cursor } = {}) {
    const params = new URLSearchParams({ type: 'chat', scope: 'all', limit: String(LIST_SESSIONS_PAGE) });
    if (cursor) params.set('cursor', cursor);
    const res = await this._spaFetch(`/api/plan-chat/v1/sessions?${params.toString()}`);
    if (!res.ok) throw new Error(`[${this.org}] sessions list HTTP ${res.status}`);
    const body = await res.json();
    const rows = Array.isArray(body) ? body : (body?.sessions ?? []);
    // Cursor pagination (backend PR): a migrated server returns {sessions,nextCursor}.
    // An OLD server returns a bare array or {sessions} with NO nextCursor field —
    // treat missing as "no more pages" and fall back to the legacy `capped`
    // lower-bound (a full page with no cursor MIGHT be truncated).
    const hasCursorField = !Array.isArray(body) && body != null &&
      Object.prototype.hasOwnProperty.call(body, 'nextCursor');
    const nextCursor = hasCursorField && typeof body.nextCursor === 'string' && body.nextCursor
      ? body.nextCursor : null;
    this.nextCursor = nextCursor;
    this.capped = !hasCursorField && rows.length >= LIST_SESSIONS_PAGE;
    ...
    const mySub = await this.operatorSub();
    ...
    const mapped = rows.map((r) => ({
      ...
      // The SPA returns `owner_email` as the owner's CF Access SUB (not an email).
      // A session is read-only ONLY when its owner matches NEITHER our sub NOR our
      // email — i.e. it's genuinely an org-mate's. Comparing our email to a sub
      // alone falsely marked every session WE own as read-only, disabling
      // steer/resume on our own sessions.
      readOnly: !!(
        r.owner_email &&
        r.owner_email !== mySub &&
        r.owner_email !== me
