// lib/agents/codex.js — CodexAdapter
//
// Concrete implementation of AgentAdapter for OpenAI Codex CLI. Handles
// transcript discovery from ~/.codex/sessions/<YYYY>/<MM>/<DD>/rollout-*.jsonl,
...
// ---------------------------------------------------------------------------
// inputSummary — replicated from lib/transcript.js (not exported there).
// Produces a short human-readable summary of a tool_use input object.
// Keep in sync with the canonical copy in lib/transcript.js:74-86.
// ---------------------------------------------------------------------------
...
// Mirrors the readTail helper in lib/transcript-index.js but reads from
// offset 0. Never throws — returns null on any error.
//
...
export const CodexAdapter = {
      ...
      await Promise.all(
        rollouts.map(async (filename) => {
          ...
          try {
            ...
            /** @type {import('./adapter.js').DiscoveredTranscript} */
            const discovered = {
              cwd: payload.cwd,
        ...
        blocks: [
          {
            ...
            input: patchInput,
            inputSummary: inputSummary({ patch: p.input }),
          },
        ],
  ...
  /**
   * No-op: Codex approval prompts are TUI-only (never written to the rollout
   * JSONL), so transcript tailing cannot surface them.
   * detectPendingFromCapture is the Codex pending channel instead.
   *
   * @param {import('./adapter.js').NormalizedMessage} _msg
   * @param {Map<string, object>} _pendingMap
   */
  trackPending(_msg, _pendingMap) {
    // Codex approval prompts are TUI-only (never written to the rollout JSONL),
    // so transcript tailing cannot surface them. detectPendingFromCapture is the
    // Codex pending channel instead. No-op here.
  },

  /**
   * Codex has no transcript-based pending detection.
   *
   * @param {string[]} _lines
   * @returns {{ transcriptPending: boolean, pendingToolUseId: null, pendingQuestion: null }}
   */
  detectTranscriptPending(_lines) {
    return { transcriptPending: false, pendingToolUseId: null, pendingQuestion: null };
  },
   ...
   * @returns {{ ctxPct: null, model: string|null }}
   */
  parseTuiStatus(capture) {
   ...
   * options. This is NOT the frontend Pending type — it is the Codex-internal
   * capture return shape used by buildAnswerProgram.
   *
   * Headings recognized:
   ...
   *
   * @param {string} capture
   * @returns {{ transcriptPending: boolean, pendingKind: string|null, header: string|null, options: Array<{n:number,label:string,shortcut:string|null,highlighted:boolean}> }}
   */
  detectPendingFromCapture(capture) {
    ...
    // Option line regex from approval-prompts.spec.json:
    //   /^\s*[›\s]\s*(\d+)\.\s+(.+?)(?:\s+\(([^)]+)\))?\s*$/
    // U+203A = ›
   ...
   *
   * @param {{ cwd?: string, bin?: string }} [opts]
   * @returns {{ bin: string, args: string[] }}
   */
  buildSpawnCommand({ cwd, bin = 'codex' } = {}) {
   ...
   *
   * @param {{ options?: Array<{n:number,label:string,highlighted:boolean}> }} pending
   * @param {string[][]} [selections]
   * @returns {string[]}
   */
  buildAnswerProgram(pending, selections) {
