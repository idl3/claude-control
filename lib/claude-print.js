...
export class ClaudePrintClient extends EventEmitter {
  ...
  _attach(socket) {
    ...
    socket.on('data', (chunk) => {
      if (this.socket === socket) this._onData(chunk);
    });
    socket.on('error', (err) => {
      if (this.socket === socket) this.emit('error', err);
    });
    socket.on('close', () => {
      // A reconnect replaces the socket before the old socket's delayed close
      // callback runs. Only the current socket generation may tear down state.
...
export class ClaudePrintManager extends EventEmitter {
  ...
  sweep(validTargets, { graceMs = 30_000 } = {}) {
    ...
    for (const [target, client] of this.clients) {
      ...
      client.close();
      // A real socket close is asynchronous; retire and announce it now while
      // ensuring the later client-level close event is treated as stale. A test
      // double may emit synchronously, in which case the bound handler already
      // performed both operations and this branch becomes a no-op.
      if (this.clients.get(target) === client) {
  ...
  _bind(client) {
    ...
    client.on('close', () => {
      // Ignore a delayed close from a client replaced at the same target. Its
      // successor owns the current transcript hint and thinking state.
