...
export class CodexRpcClient extends EventEmitter {
  constructor({ target, endpoint, cwd, resumeThreadId = null, transcriptPath = null, model = null, skipPermissions = false }) {
    super();
    this.target = target;
    ...
    // Same "fresh thread only" rule as model above — a resumed thread keeps
    // whatever approval/sandbox policy it was already running under.
  ...
  async _openThread() {
    const startParams = removeNullish({
      ...
      model: this.model,
      // approvalPolicy/sandbox: RPC equivalent of the CLI's
      // --dangerously-bypass-approvals-and-sandbox flag (there is no such
      // app-server CLI flag — this is the documented RPC param pair, per
      // the generated ThreadStartParams JSON schema). 'never' + 'danger-
      // full-access' fully bypass approval prompts and sandboxing.
      approvalPolicy: this.skipPermissions ? 'never' : undefined,
      sandbox: this.skipPermissions ? 'danger-full-access' : undefined,
    });
    const resumeParams = {
      cwd: this.cwd,
    };
...
export class CodexRpcManager extends EventEmitter {
  ...
  async attach({ target, endpoint, cwd, resumeThreadId = null, transcriptPath = null, model = null, skipPermissions = false }) {
    const existing = this.clients.get(target);
    if (existing) existing.close();
    const client = new CodexRpcClient({ target, endpoint, cwd, resumeThreadId, transcriptPath, model, skipPermissions });
    this._bind(client);
    try {
