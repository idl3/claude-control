...
function _loadNewRunCached(runDir, runId, scriptsDir, scriptNames) {
  ...
  _rememberMapEntry(_wfNewCache, runDir, { mtimeMs: stat.mtimeMs, size: stat.size, run }, WF_CACHE_MAX_KEYS);
  return run;
}
...
function _buildNewRun(runDir, runId, journalPath, journalStat, scriptsDir, scriptNames) {
  ...
  // First-seen order = journal (start) order. `started` registers an agent (or a
  // stray `result` does, defensively); `result` advances it to done and carries
  // its structured return. Unknown event types are ignored for state.
  const agents = new Map(); // agentId → {agentId, hasResult, result}
  for (const line of raw.split('\n')) {
    ...
    if (!a) {
      a = { agentId, hasResult: false, result: null };
  ...
  const { workflowName, summary } = _resolveScriptMeta(runId, scriptsDir, scriptNames);
  ...
  for (const a of agents.values()) {
    index += 1;
    const state = a.hasResult ? 'done' : 'running'; // started→running, result→done
    if (state === 'done') done += 1;
    shaped.push({
      index,
      label: null,
      agentId: a.agentId,
      agentType: _readAgentType(runDir, a.agentId),
      model: _sniffAgentModel(runDir, a.agentId),
      state,
      startedAt: null,
      queuedAt: null,
      durationMs: null,
  ...
  // Fail-open to 'running': a run with zero agents yet, or ANY agent still
  // lacking its result, must never read as finished (subagents' "under-complete
  // rather than falsely done" discipline). Only an all-done run is 'completed'.
  const status = total > 0 && shaped.every((ag) => ag.state === 'done') ? 'completed' : 'running';
...
/**
 * Resolve `{workflowName, summary}` for a run from its script `<name>-<runId>.js`.
 * workflowName comes from the FILENAME (strip the runId suffix — robust, no JS
 * parsing); summary is a bounded, best-effort regex of the script's
 * `meta.description`. Both null when the script is absent/unreadable.
 */
function _resolveScriptMeta(runId, scriptsDir, scriptNames) {
  const file = scriptNames.find((n) => n.endsWith('.js') && n.includes(runId));
  if (!file) return { workflowName: null, summary: null };
  ...
  try {
    ...
    const m = buf.toString('utf8', 0, n).match(/description:\s*'((?:\\.|[^'\\])*)'/);
    if (m) summary = _truncate(m[1].replace(/\\'/g, "'"), PREVIEW_MAX);
  } catch {
    /* no script head → summary stays null */
  } finally {
    if (fd !== undefined) {
      try {
        fs.closeSync(fd);
