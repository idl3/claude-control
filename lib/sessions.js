...
export class SessionRegistry extends EventEmitter {
  ...
  async _doRefresh() {
    ...
    if (codexPanes.length > 0) {
      // --- Phase 1: lsof authoritative binding (concurrent) ------------------
      const lsofResults = await Promise.all(
        codexPanes.map(async (p) => {
          try {
            const procInfo = paneProc.get(p.target);
            const codexPid = procInfo?.pid ?? null;
            if (!codexPid) return { target: p.target, rec: null };
            const rolloutPath = await findOpenRollout(codexPid);
            if (!rolloutPath) return { target: p.target, rec: null };
            const rec = await readRolloutMeta(rolloutPath);
            return { target: p.target, rec };
          } catch {
            return { target: p.target, rec: null };
          }
        }),
      );

      // Apply lsof results — these win over any heuristic.
      const lsofResolved = new Set();
      for (const { target, rec } of lsofResults) {
        if (rec) {
          assignment.set(target, rec);
          lsofResolved.add(target);
        }
      }

      // --- Phase 2: heuristic fallback for unresolved panes ------------------
      const unresolved = codexPanes.filter((p) => !lsofResolved.has(p.target));
      if (unresolved.length > 0) {
        ...
        // Only apply heuristic results to panes not already bound by lsof.
        for (const [t, rec] of codexAssignment) {
          if (!lsofResolved.has(t)) assignment.set(t, rec);
        }
      }
    }
    // ── End Codex matching ───────────────────────────────────────────────────
    ...
    const sessions = panes.map((win) => {
      ...
      const panePrompt = (isClaude || kind === 'codex') ? this._panePromptMap.get(id) : null;
      const pending =
    ...
    const findClaude = (rootPid) => {
      ...
      while (queue.length) {
        ...
        if (meta && CODEX_COMM_RE.test(meta.comm)) {
          const sec = parseEtime(meta.etime);
          return { isClaude: false, isCodex: true, kind: 'codex', startMs: sec == null ? null : now - sec * 1000, pid };
        }
        for (const c of children.get(pid) ?? []) queue.push(c);
      }
      return { isClaude: false, isCodex: false, kind: null, startMs: null, pid: null };
    };
