...
// and its claudex variant (the claude binary pointed at the olam
// auth-worker via ANTHROPIC_BASE_URL, see server.js's /api/session/new
// claudex branch). The two run the identical claude binary, so the
// transcript format, TUI scraping, and effort/context reporting are
// identical — only the upstream endpoint differs. Every claude-pane
// behavior gate should accept both. Also valid for the raw `@cc_agent` tmux
// pane-option tag, which shares the same value space ('claude' | 'codex' |
// 'claudex' | null). Exported so server.js's picker-guard/transport gates
// share this single source of truth instead of re-deriving it.
export function isClaudeKind(kind) {
  return kind === 'claude' || kind === 'claudex';
}
...
export class SessionRegistry extends EventEmitter {
  ...
  async _doRefresh() {
    ...
    const paneKind = (p) => {
      ...
      // Checked BEFORE the ps-based fallback below: claudex spawns the real
      // claude binary, so without this the process scan would misclassify
      // it as plain 'claude' (losing the first-class kind — see
      // isClaudeKind's doc comment for why the two need to stay distinct
