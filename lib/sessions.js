 ...
 * The record's cwd is the pane's LAUNCH path (== the tmux pane's current path),
 * so a correct binding has record.cwd === pane.cwd EXACTLY. tmux recycles %N
 * across a server restart, so a record whose cwd differs is a stale binding for
 * a DIFFERENT session — reject it (unlike isCwdConsistent, no descendant
 * leniency: `pleri-org/olam` must NOT match a live `pleri-org` pane, which is
 * exactly the reused-%N mismatch this guards).
 *
 ...
 * @returns {boolean}
 */
export function isPinSuperseded(entry, reg, paneCwd) {
  if (!reg?.transcriptPath) return false;
  if (!paneRecordMatchesCwd(reg.cwd, paneCwd)) return false; // reused %N — reg is the stale one
...
export class SessionRegistry extends EventEmitter {
  ...
  async _doRefresh() {
    ...
    for (const p of claudePanes) {
      ...
      if (!pinned) continue;
      if (isPinSuperseded(entry, p.paneId ? paneReg.get(p.paneId) : null, p.cwd)) continue;
    ...
    for (const p of claudePanes) {
      ...
      // restart, so a record whose recorded launch cwd no longer matches the live
      // pane's cwd is a stale binding for a DIFFERENT session. Ignoring it here
      // stops the hook path from binding the wrong transcript (the reused-%N
      // mismatch — e.g. an old olam record on a now-pleri-org pane).
      const reg = regRaw && paneRecordMatchesCwd(regRaw.cwd, p.cwd) ? regRaw : null;
