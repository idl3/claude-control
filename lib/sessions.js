    ...
    await Promise.all(
      sessions.map(async (s) => {
        ...
        try {
          ...
          // Backfill effort even while idle: a Claude pane with no effort yet (its
          // statusLine hadn't re-rendered with the effort field when last scraped)
          // must still be scraped so it can join the model/effort/ctx rotation like
          // every other row. Once effort is known it's sticky (below), so this
          // exception stops firing; `refreshInterval` in the statusLine config keeps
          // idle panes' statuslines fresh so the scrape actually finds the effort.
          const needEffortBackfill = s.kind === 'claude' && !s.effort;
          if (
