...
export class SessionRegistry extends EventEmitter {
  ...
  async _doRefresh() {
    ...
    if (codexPanes.length > 0) {
        ...
        codexPanes.map(async (p) => {
          try {
            ...
            if (hint?.transcriptPath) {
              const rec = await readCodexTranscriptRecord(hint.transcriptPath);
              if (rec) {
                exactByTarget.set(p.target, codexRecordToCandidate({
                  ...rec,
                  sessionId: rec.sessionId ?? hint.sessionId ?? null,
                }));
            ...
            if (!rolloutPath || exactPaths.has(rolloutPath)) return;
      ...
      const unresolved = codexPanes.filter((p) => !exactByTarget.has(p.target));
  ...
  async _buildPaneProc(allPanes) {
    ...
    const findClaude = (rootPid) => {
      ...
      while (queue.length) {
        ...
        if (meta && (CODEX_COMM_RE.test(meta.comm) || codexMatchesProcess(meta.args))) {
          const sec = parseEtime(meta.etime);
