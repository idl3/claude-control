...
export async function extractTailRecord(filePath, mtime, birthtime = null) {
  ...
  const base = {
    ...
    // assistant record ended with a terminal stop_reason AND no user
    // tool_result follows it (which would mean the agent is mid-tool-loop).
    turnComplete: false,
  };
  ...
  // extra cost). Walking newest-first, the FIRST assistant record we hit is the
  // newest one; any user tool_result seen BEFORE it is "later" in file order.
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    try { rec = JSON.parse(line); } catch { continue; }
    if (!rec || typeof rec !== 'object') continue;
    // Turn-completion capture (before the newest assistant record is found):
    // a trailing user tool_result means the agent is mid-tool-loop, not done.
    if (!assistantSeen) {
      if (rec.type === 'assistant') {
        ...
        rec.message.content.some((b) => b && typeof b === 'object' && b.type === 'tool_result')
      ) {
  ...
  base.turnComplete =
    TURN_COMPLETE_STOP_REASONS.has(newestAssistantStopReason ?? '') && !sawTrailingToolResult;
  return base;
}
...
export class SessionRegistry extends EventEmitter {
   ...
   *
   *  - Claude: the transcript's newest assistant turn ENDED (terminal
   *    stop_reason, no trailing tool_result) — `s.turnComplete`.
   *  - Codex: no such JSONL signal, so mtime-stability is the evidence — its
   *    rollout keeps being written while the turn is live, so a quiet transcript
  ...
  _turnDefinitivelyIdle(s, activeUntil) {
    if (!(isClaudeKind(s.kind) || s.kind === 'codex')) return false;
    ...
    // Claude: definitive turn-end from the transcript. Codex: the mtime-stable
    // gate above is the evidence (its rollout keeps writing while it works).
    if (isClaudeKind(s.kind)) return s.turnComplete === true;
    return true; // codex
  }
