...
import { parseTuiStatus, prettyModel, resolveModelLabel } from './tui.js';
...
import { pinKey, pinPath, pinTs } from './pins.js';
import { readPaneRegistry, gcPaneRegistry, writePaneRegistryRecord } from './pane-registry.js';
import {
...
import { computeSubAgentActivity } from './subagents.js';
import { computeWorkflowActivity, deriveWorkflowSummary } from './workflows.js';
import { normalizeRoots } from './projects-roots.js';
import { EPHEMERAL_SESSION_PREFIX } from './tmux.js';
import { matchesProcess as codewhaleMatchesProcess } from './codewhale.js';
...
// True for any Claude-flavored pane/session kind — the native 'claude' pane
// and its claudex/claudemi variants (the claude binary pointed at the olam
// auth-worker via ANTHROPIC_BASE_URL, see server.js's /api/session/new
// claudex/claudemi branches). All three run the identical claude binary, so
// the transcript format, TUI scraping, and effort/context reporting are
// identical — only the upstream endpoint differs (claudex → OpenAI, claudemi
// → Kimi via the worker's /kimi provider-selector segment). Every
// claude-pane behavior gate should accept all three. Also valid for the raw
// `@cc_agent` tmux pane-option tag, which shares the same value space
// ('claude' | 'codex' | 'claudex' | 'claudemi' | null). Exported so
// server.js's picker-guard/transport gates share this single source of
// truth instead of re-deriving it.
export function isClaudeKind(kind) {
  return kind === 'claude' || kind === 'claudex' || kind === 'claudemi';
}
...
// How long a _pollThinking-captured codex prompt stays trustworthy for reuse by
// server.js's startPromptPoller (R9 dedup). Slightly wider than the poll cycle
// itself to absorb timer jitter between the two independent 2 s interval loops.
const PANE_PROMPT_FRESH_MS = 2500;
// gcPaneRegistry() only deletes registry files whose transcript is already
// gone — nothing time-sensitive depends on it running every 4 s refresh tick.
// Throttled to once every N _doRefresh() cycles (R11, same cycle-counter idiom
// as SELFHEAL_DEBOUNCE_CYCLES below) instead of every call, cutting a
// redundant readdir+parse+existsSync pass over ~/.claude-control/panes/ by 80%
// while readPaneRegistry() keeps running every cycle (its result feeds the
// pane→transcript match this same tick).
const PANE_GC_INTERVAL_CYCLES = 5; // ~20 s at the default 4 s REFRESH_INTERVAL_MS
...
/**
 * May a transcript record retire the sticky tailer-set `pending` flag?
 *
 * `_pendingMap` is written ONLY by `tailer.on('pending')` (server.js), and a
 * tailer exists only while a client is watching that session. Close the view
 * with a question open, answer it in the terminal, and no `pending:false` event
 * is ever emitted — the "awaiting your answer" badge then sticks for the life of
 * the process, even though the transcript recorded the answer immediately.
 *
 * Only a POSITIVE verdict retires it. `transcriptPending` is a real boolean
 * (`pendingToolUseId !== null`), but the whole record is null when the
 * transcript could not be read or matched; treating that unknown as "answered"
 * would silently drop a REAL question — the one failure direction that actually
 * costs the user. So: `=== false`, never merely falsy.
 *
 * @param {{transcriptPending?: boolean}|null|undefined} transcript
 * @returns {boolean}
 */
export function shouldRetireStickyPending(transcript) {
  return !!transcript && transcript.transcriptPending === false;
}

/**
 * Fallback-only: does a record's recorded launch cwd equal the pane's current
 * path? Used by paneRecordBelongsToPane when the pane's process start time is
 * unavailable. NOT a reliable identity test on its own — see that function.
 *
 * @param {string|null} recCwd record's recorded launch cwd
 * @param {string} winCwd      live tmux pane current path
 * @returns {boolean}
 */
export function paneRecordMatchesCwd(recCwd, winCwd) {
  if (!recCwd) return true; // legacy record without cwd — nothing to contradict
  const norm = (c) => String(c).replace(/\/$/, '');
  return norm(recCwd) === norm(winCwd);
}

// `startMs` is derived from `ps` elapsed SECONDS, so it can land up to a second
// late; the SessionStart hook also writes its record a moment after launch.
// Tolerance well above both, and still orders-of-magnitude below the gap in a
// real recycled-%N case (that record belongs to a whole previous tmux server).
const PANE_RECORD_SKEW_MS = 10_000;

/**
 * Does a pane-registry record still belong to the live pane holding its %N?
 *
 * tmux recycles %N across a server restart, so a record from a previous
 * generation can name a pane that now runs something else entirely. This used
 * to be judged by requiring `record.cwd === pane.cwd` exactly — but tmux's
 * `pane_current_path` is LIVE: it follows the foreground process, so an agent
 * that `cd`s out of its launch directory stops matching its OWN record. That
 * is a false negative with no recovery: the deterministic hook binding is
 * dropped, the auto-matcher has nothing to fall back to, and the session
 * renders with no transcript at all. Observed on two long-running agents that
 * had cd'd from `pleri-org/olam` up to `pleri-org`.
 *
 * Time is the signal that actually distinguishes the two cases. A record for
 * THIS pane is written by the agent running in it, so it cannot predate that
 * agent's process; a record left on a recycled %N was authored before the
 * current process existed. cwd equality stays only as the fallback for panes
 * whose process start can't be read (`ps` failure / non-agent pane).
 *
 * @param {{cwd?: string|null, ts?: number}|null} rec pane-registry record
 * @param {string} paneCwd       live tmux pane current path
 * @param {number|null} procStartMs epoch ms the pane's agent process started
 * @returns {boolean}
 */
export function paneRecordBelongsToPane(rec, paneCwd, procStartMs) {
  if (!rec) return false;
  if (typeof procStartMs === 'number' && typeof rec.ts === 'number') {
    return rec.ts >= procStartMs - PANE_RECORD_SKEW_MS;
  }
  return paneRecordMatchesCwd(rec.cwd, paneCwd);
}

/**
 * Has a manual pin been overtaken by the SessionStart hook? A pin overrides the
 * heuristic MATCHER, not ground truth: when the hook later records a different
 * transcript for the same live pane, that pane started a new session and the
 * pin now points at a dead one. A pin set AFTER the hook record still wins —
 * that's the operator deliberately re-binding a correctly-matched pane.
 *
 * Legacy pins are bare strings with no ts (→ 0), so any hook record supersedes
 * them; that is what unwedges panes pinned before pins carried a timestamp.
 *
 * @param {string|{path:string,ts?:number}} entry pin entry
 * @param {{transcriptPath:string, cwd?:string|null, ts?:number}|null} reg hook record for this pane
 * @param {string} paneCwd live tmux pane cwd
 * @param {number|null} [procStartMs] epoch ms the pane's agent process started
 * @returns {boolean}
 */
export function isPinSuperseded(entry, reg, paneCwd, procStartMs = null) {
  if (!reg?.transcriptPath) return false;
  // Same identity test the hook binding uses — a cd'd agent must not be mistaken
  // for a recycled %N here either, or a stale pin would win again.
  if (!paneRecordBelongsToPane(reg, paneCwd, procStartMs)) return false;
  if (reg.transcriptPath === pinPath(entry)) return false; // they agree
  return (reg.ts ?? 0) > pinTs(entry);
}

/**
 * Sum the RSS (in MB) of a pid and all its descendants, given a parsed process
 * table. A pane's memory is the whole subtree under its shell — claude + its MCP
 * servers, node, caffeinate, etc. — not just the foreground process.
 *
 * @param {number|string} rootPid
 * @param {{children: Map<string,string[]>, rssKb: Map<string,number>}} table
 * @returns {number|null} MB (rounded), or null when the pid isn't in the table.
 */
export function sumTreeRssMB(rootPid, table) {
  const root = String(rootPid);
  if (!table.rssKb.has(root)) return null;
  let totalKb = 0;
  const seen = new Set();
  const stack = [root];
  while (stack.length) {
    const pid = stack.pop();
    if (seen.has(pid)) continue; // cycle guard (ppid loops shouldn't happen, but be safe)
    seen.add(pid);
    totalKb += table.rssKb.get(pid) ?? 0;
    for (const c of table.children.get(pid) ?? []) stack.push(c);
  }
  return Math.round(totalKb / 1024);
}

/**
 * One `ps` snapshot → an index for sumTreeRssMB. Called once per sessions refresh
 * so per-session memory is O(processes) total, not one exec per session.
 * Best-effort: returns an empty index (all memMB null) if ps fails.
 */
export async function buildProcMemTable() {
  const empty = { children: new Map(), rssKb: new Map() };
  try {
    const { stdout } = await execFile('ps', ['-A', '-o', 'pid=,ppid=,rss=']);
    const children = new Map();
    const rssKb = new Map();
    for (const line of stdout.split('\n')) {
      const m = line.trim().match(/^(\d+)\s+(\d+)\s+(\d+)$/);
      if (!m) continue;
      const [, pid, ppid, rss] = m;
      rssKb.set(pid, Number(rss));
...
// A Claude pane that has no effort yet is scraped once-per-poll to backfill it —
// but ONLY while it was recently active (used within this window), never for
// long-idle panes. This keeps the "idle → zero scrape" battery invariant for
// truly-idle panes and avoids perpetually re-scraping a pane whose statusLine
// simply never reports an effort (unsupported model / stale render).
const EFFORT_BACKFILL_WINDOW_MS = 10 * 60_000;
...
export function detectTranscriptPending(lines) {
  // A question is pending ONLY while it is the transcript's FINAL turn — i.e. the
  // agent asked and is still waiting. Any later `user` record ends the wait,
  // whether the human selected an option (recorded as a matching `tool_result`)
  // OR typed a free-text reply (which Claude Code records with NO tool_result for
  // the AskUserQuestion). The old tool_use↔tool_result pairing left a typed-over
  // question dangling "open" forever, so the session read as perpetually pending
  // long after it was answered. Tracking the newest open question and clearing it
  // on the next user turn captures both the selection and the typed-reply cases.
  ...
  for (const raw of lines) {
    ...
    // The human is back in control → nothing is pending (answered or moved past).
    if (rec.type === 'user') {
    ...
    for (const block of content) {
      ...
      if (
        ...
        pendingQuestion = typeof q === 'string' ? q : null;
      }
    }
  }
...
// ---------------------------------------------------------------------------
// Context-window % — derived from real transcript token usage instead of
// scraping the TUI's "ctx:NN%" status line (see lib/tui.js CTX_RE). The TUI
// scrape is missing for Codex/olam-remote panes and for any Claude pane whose
// status line wasn't in the captured region, and shows a stale/meaningless
// value for idle or just-compacted sessions. The transcript's per-turn
// `message.usage` block is always present for a real Claude turn, so it's a
// more reliable source; the TUI value is kept only as a fallback.
const DEFAULT_CONTEXT_WINDOW = 200_000;
const EXTENDED_CONTEXT_WINDOW = 1_000_000; // Opus 1M-context beta
const ONE_M_LABEL_RE = /1M context/i;
// Models whose NATIVE context window is 1M, independent of any label decoration.
//
// The label regex above only ever matches a TUI-parsed label ("Opus 4.8 (1M
// context)"). When a session's model is reported as a raw id instead — which
// happens whenever prettyModel() can't parse it — no label carries that marker,
// so the window silently fell back to 200k and then to the usedTokens heuristic
// below. For fable that produced a meter which counted DOWN to 4% at 191k tokens
// and then JUMPED UP to ~81% the moment the session crossed 200k, because only
// then did the heuristic switch windows. Observed live: four claude-fable-5
// sessions at 191,794 / 254,271 / 258,994 / 749,729 tokens, the first measured
// against 200k and the rest against 1M — same model, two different windows.
//
// Matching on a family keyword (not an exact id) means this holds whether the
// caller passes the raw id ("claude-fable-5") or the pretty label ("Fable 5").
const ONE_M_MODEL_RE = /\bfable\b/i;

/**
 * Sum of the token categories that count against the model's context window
 * for a single turn: fresh input + newly-cached input + tokens served from
 * cache. Output tokens are generated, not loaded context, so they're
 * excluded.
 *
 * @param {object} usage  an assistant message's `message.usage` block
 * @returns {number}
 */
...
/**
 * Resolve the context-window size (tokens) for a session. The raw model id
 * recorded in the transcript (e.g. "claude-opus-4-8") carries no window
 * annotation — the 1M-context beta is enabled per-request via a header, not a
 * distinct model id — so it can't be read off the transcript alone. Detect it
 * two ways: (1) the TUI-scraped model label, when available, spells out
 * "(1M context)" verbatim; (2) failing that, a used-token count that exceeds
 * the default 200k window is only possible under the extended window, so the
 * usage figure itself is proof.
 *
 * @param {string|null} modelLabel
 * @param {number|null} usedTokens
 * @returns {number}
 */
function resolveContextWindow(modelLabel, usedTokens) {
  if (modelLabel && ONE_M_LABEL_RE.test(modelLabel)) return EXTENDED_CONTEXT_WINDOW;
  // Native-1M model (e.g. fable): correct BEFORE the heuristic below, so the
  // window no longer depends on how full the session happens to be.
  if (modelLabel && ONE_M_MODEL_RE.test(modelLabel)) return EXTENDED_CONTEXT_WINDOW;
  if (typeof usedTokens === 'number' && usedTokens > DEFAULT_CONTEXT_WINDOW) return EXTENDED_CONTEXT_WINDOW;
  return DEFAULT_CONTEXT_WINDOW;
}

/**
 * Context-remaining percentage from real transcript token usage.
 *
 * SEMANTIC: this mirrors the TUI's existing "ctx:NN%" meaning — NN is room
 * REMAINING (higher = more headroom), not tokens consumed. A nearly-full
 * session reads LOW; a fresh session reads ~100.
 *
 * @param {number|null} usedTokens  context tokens loaded on the last turn, or null if unknown
 * @param {string|null} modelLabel  display model label, used only for 1M-context detection
 * @returns {number|null} 0-100, or null when usage is unavailable
 */
export function computeCtxPctFromUsage(usedTokens, modelLabel) {
  if (typeof usedTokens !== 'number' || !Number.isFinite(usedTokens) || usedTokens < 0) return null;
  const window = resolveContextWindow(modelLabel, usedTokens);
  const pct = Math.round(100 * (1 - usedTokens / window));
  return Math.max(0, Math.min(100, pct));
}

// Assistant stop_reasons that mean the turn ENDED (the agent handed control
// back to the user). `tool_use` (agent is calling a tool, more work coming)
// and `pause_turn` (a long-running turn the agent will resume) are DELIBERATELY
// excluded — those are mid-turn, not complete. Used to derive `turnComplete`,
// the hard JSONL evidence the registry uses to clear a stale "working…" flag.
const TURN_COMPLETE_STOP_REASONS = new Set(['end_turn', 'stop_sequence', 'max_tokens']);
...
export async function extractTailRecord(filePath, mtime, birthtime = null) {
  ...
  const base = {
    ...
    // Hard turn-end evidence for the stale "working…" indicator: the NEWEST
    // main-line conversation record is a terminal assistant turn (stop_reason ∈
    // end_turn/stop_sequence/max_tokens). If ANY main-line record is newer than
    // it — a fresh user turn, or a pending tool_result — the agent is still
    // working, so this stays false. Sub-agent (sidechain) records are skipped
    // entirely; a sidechain's end_turn is not the main turn's end.
    turnComplete: false,
  };
  ...
  // Turn-completion evidence, computed on the SAME end→start walk (near-zero
  // extra cost). We inspect only MAIN-LINE conversation records (type user or
  // assistant, isSidechain !== true) newest-first, and the FIRST one we reach
  // decides the turn:
  //   - assistant  → capture its stop_reason; turnComplete iff it is terminal.
  //   - user       → a user message NEWER than any assistant means a fresh turn
  //                  began, or a tool_result is pending (mid-tool-loop). Either
  //                  way the agent is still working → NOT complete.
  // Sub-agent (sidechain) records are transparent to this scan: a sidechain
  // end_turn must never be mistaken for the main turn's end (P3), and a
  // sidechain tool_result must never break a completed main turn. Metadata
  // records (ai-title, custom-title, summary, system, …) are ignored — only
  // user/assistant conversation records decide.
  let turnDecided = false;
  let newestAssistantStopReason = null;
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    try { rec = JSON.parse(line); } catch { continue; }
    if (!rec || typeof rec !== 'object') continue;
    // Turn-completion capture: first main-line conversation record wins.
    if (!turnDecided && rec.isSidechain !== true) {
      if (rec.type === 'assistant') {
        const sr = rec.message?.stop_reason;
        newestAssistantStopReason = typeof sr === 'string' ? sr : null;
        turnDecided = true;
      } else if (rec.type === 'user') {
        // Newest main-line record is a user turn/tool_result → not idle. Leaves
        // newestAssistantStopReason null, so turnComplete resolves to false, and
        // stops the scan so an OLDER assistant end_turn can't be mispicked.
        turnDecided = true;
      }
    }
    ...
    // Last assistant message's usage = the context actually loaded on the
    // most recent turn. Walking end-to-start, the first assistant record we
    // hit with a usage block IS the last one in the file — no separate pass
    // needed. Robust to a truncated/partial first line (tail read artifact)
    // since JSON.parse already skips unparsable lines above.
    if (base.contextUsedTokens === null && rec.type === 'assistant' && rec.message?.usage) {
      base.contextUsedTokens = usageContextTokens(rec.message.usage);
    }
  ...
  base.turnComplete = TURN_COMPLETE_STOP_REASONS.has(newestAssistantStopReason ?? '');
  return base;
}
...
export class SessionRegistry extends EventEmitter {
   ...
   */
  constructor({ projectsRoot, projectsRoots, codexSessionsRoot, tmux, debounceMs = 1000, findOpenRollout: findOpenRolloutOverride } = {}) {
    super();
    this._projectsRoots = normalizeRoots(projectsRoots, projectsRoot);
    this._codexSessionsRoot = codexSessionsRoot;
    this._tmux = tmux;
    this._debounceMs = debounceMs;
    // Injectable so the lsof-based codex binding is testable without a real
    // codex process; defaults to the real lsof implementation.
    this._findOpenRollout = findOpenRolloutOverride || findOpenRollout;

    /** @type {Record<string,string>} pin key (windowId.paneIndex) -> transcript path */
    this._pins = {};
    /** @type {Session[]} */
    this._sessions = [];
    /** @type {Map<string, Session[]>} — rows owned by external session adapters */
    this._externalSessions = new Map();
    ...
    /** @type {Map<string, {ctxPct:number|null, model:string|null, effort:string|null}>} target -> TUI status */
    ...
    /** @type {Map<string, number>} target -> count of currently-running sub-agents */
    this._subAgentCountMap = new Map();
    /** @type {Map<string, object[]>} target -> parsed workflow runs (see lib/workflows.js) */
    this._workflowsMap = new Map();
    /** @type {Map<string, boolean>} target -> has a workflow run actively running */
    this._workflowActiveMap = new Map();
    /** @type {Map<string, object|null>} target -> {name, activePhaseTitle, done, total, status} */
    this._workflowSummaryMap = new Map();
    /** @type {Map<string, {pending:boolean, question:string|null}>} target -> pane-derived prompt */
    this._panePromptMap = new Map();
    /** @type {Map<string, {prompt:{question:string,options:Array}|null, at:number}>} target -> full parseCodexPrompt() result cached by _pollThinking, for getPanePrompt() reuse (R9 dedup) */
    this._codexPromptCache = new Map();
    ...
    /** @type {Map<string, string|null>} target -> stable pane id from last refresh */
    this._paneIdentityByTarget = new Map();
    ...
    /** @type {Promise<Session[]>|null} — the in-flight refresh() call, if any */
  ...
  _externalRows() {
    return [...this._externalSessions.values()].flat();
  }

  /**
   * Replace one adapter's externally-discovered session rows without touching
   * local tmux rows or another adapter's rows. This is the session-source seam:
   * Olam and CodeWhale Runtime both publish the same Session-shaped interface.
   *
   * @param {string} source
   * @param {Session[]} rows
   */
  setExternalSessions(source, rows) {
    if (typeof source !== 'string' || !source.trim()) throw new Error('external session source is required');
    const oldIds = new Set(this._externalRows().map((session) => session.id));
    const normalizedRows = (Array.isArray(rows) ? rows : []).map((row) => {
      const panePrompt = this._panePromptMap.get(row.id) ?? null;
      return {
        ...row,
        pending: !!row.pending || (this._pendingMap.get(row.id) ?? false) || !!panePrompt?.pending,
        pendingQuestion: row.pendingQuestion ?? panePrompt?.question ?? null,
        subAgentActive: row.subAgentActive ?? (this._subAgentActiveMap.get(row.id) ?? false),
        runningSubagentCount: row.runningSubagentCount ?? (this._subAgentCountMap.get(row.id) ?? 0),
      };
    });
    this._externalSessions.set(source, normalizedRows);
    this._sessions = this._sessions
      .filter((session) => !oldIds.has(session.id))
      .concat(this._externalRows());
    this._maybeEmit();
  }

  /** Backward-compatible Olam adapter entrypoint. */
  setRemoteSessions(rows) {
    this.setExternalSessions('olam', rows);
  }
  ...
  /** Publish structured sub-agent activity from a non-filesystem substrate. */
  setSubAgentState(id, count) {
    const running = Math.max(0, Number(count) || 0);
    this._subAgentCountMap.set(id, running);
    this._subAgentActiveMap.set(id, running > 0);
    const session = this._sessions.find((candidate) => candidate.id === id);
    if (!session) return;
    const changed = session.subAgentActive !== (running > 0)
      || session.runningSubagentCount !== running;
    session.subAgentActive = running > 0;
    session.runningSubagentCount = running;
    if (changed) this._maybeEmit();
  }

  /**
   * Return the last parseCodexPrompt() result _pollThinking captured for a
   * codex pane target, plus whether it's still fresh enough to trust without
   * re-capturing. Lets server.js's per-subscription startPromptPoller skip its
   * own tmux capture-pane call for codex sessions when _pollThinking already
   * ran the SAME capture this cycle (R9 dedup).
   *
   * "Fresh" means captured within PANE_PROMPT_FRESH_MS. A stale/absent entry
   * means the pane was idle-gated by shouldScrapePane on the current
   * _pollThinking tick (no active generation/transcript activity) — the
   * caller MUST fall back to its own capture in that case, since a Codex
   * approval prompt can appear on an otherwise-idle pane with no preceding
   * transcript write.
   *
   * @param {string} target
   * @returns {{ prompt: {question:string, options:Array<{key:string,label:string,selected:boolean}>}|null, fresh: boolean }}
   */
  getPanePrompt(target) {
    const entry = this._codexPromptCache.get(target);
    if (!entry) return { prompt: null, fresh: false };
    return { prompt: entry.prompt, fresh: Date.now() - entry.at < PANE_PROMPT_FRESH_MS };
  }
  ...
  _pruneTargetState(panes) {
    const liveTargets = new Set([
      ...panes.map((p) => p.target),
      ...this._externalRows().map((s) => s.id),
    ]);
    const targetMaps = [
      this._pendingMap,
      this._ctxMap,
      ...
      this._erroredMap,
      this._permIssueMap,
      this._subAgentActiveMap,
      this._subAgentCountMap,
      this._workflowsMap,
      this._workflowActiveMap,
      this._workflowSummaryMap,
      this._panePromptMap,
      this._codexPromptCache,
      this._transcriptHintMap,
      this._paneTextCache,
      this._healLastCycle,
    ];
    ...
    for (const map of targetMaps) {
      for (const target of map.keys()) {
        if (!liveTargets.has(target)) map.delete(target);
      }
    }
    for (const target of this._paneIdentityByTarget.keys()) {
      if (!liveTargets.has(target)) this._paneIdentityByTarget.delete(target);
    }
    for (const pane of panes) {
      const priorPaneId = this._paneIdentityByTarget.get(pane.target);
      if (priorPaneId !== undefined && priorPaneId !== (pane.paneId ?? null)) {
        // tmux reused a numeric target (for example 0:1.0) for a new pane. Any
        // target-keyed hint/status belongs to the old process and must not leak.
        for (const map of targetMaps) map.delete(pane.target);
      }
      this._paneIdentityByTarget.set(pane.target, pane.paneId ?? null);
    }
  }
   ...
   */
  async refresh() {
    if (this._refreshing) return this._refreshInFlight;
  ...
  /**
   * Force a fresh rebuild, guaranteed to reflect any tmux mutation made just
   * before this call — unlike refresh(), which no-ops (returning the in-flight
   * promise) when a periodic tick is already running and may have started
   * before that mutation happened. Awaits any in-flight pass first (so its
   * shellouts don't race ours), then always runs a brand-new _doRefresh().
   *
   * @returns {Promise<Session[]>}
   */
  async refreshNow() {
    if (this._refreshInFlight) {
      await this._refreshInFlight.catch(() => {});
    }
    return this.refresh();
  }
  ...
  async _doRefresh() {
    ...
    // A tmux shellout can fail transiently while the server/socket is busy or
    // being recreated. Treat that differently from a successful empty result:
    // pruning against [] here would erase every target-keyed hint/status and
    // briefly make all local sessions disappear. The next periodic refresh can
    // reconcile from a trustworthy snapshot.
    if (allPanes === null) return this._sessions;
    ...
    // An ephemeral pty-bridge mirror session (EPHEMERAL_SESSION_PREFIX) can
    // transiently list its mirrored pane before/after the real session name
    // does — never let it win the dedup and mask the real session.
    const seenPaneIdx = new Map(); // `${windowId}.${paneIndex}` -> index in `panes`
    const panes = [];
    for (const p of allPanes) {
      const key = `${p.windowId}.${p.paneIndex}`;
      const idx = seenPaneIdx.get(key);
      if (idx === undefined) {
        seenPaneIdx.set(key, panes.length);
        panes.push(p);
        continue;
      }
      const kept = panes[idx];
      const keptIsEphemeral = kept.sessionName?.startsWith(EPHEMERAL_SESSION_PREFIX);
      const candidateIsEphemeral = p.sessionName?.startsWith(EPHEMERAL_SESSION_PREFIX);
      if (keptIsEphemeral && !candidateIsEphemeral) {
        panes[idx] = p; // prefer the real session over the ephemeral mirror
      }
    }
    this._pruneTargetState(panes);
    ...
    const isClaudePane = (p) => {
      if (isClaudeKind(p.ccAgent)) return true;
      const info = paneProc.get(p.target);
      return info ? info.isClaude : isClaudeCmd(p.cmd);
    };
    const paneKind = (p) => {
      if (p.ccAgent === 'claude') return 'claude';
      // Checked BEFORE the ps-based fallback below: claudex/claudemi spawn
      // the real claude binary, so without this the process scan would
      // misclassify them as plain 'claude' (losing the first-class kind —
      // see isClaudeKind's doc comment for why these need to stay distinct
      // kinds even though most gates treat them identically).
      if (p.ccAgent === 'claudex') return 'claudex';
      if (p.ccAgent === 'claudemi') return 'claudemi';
      if (p.ccAgent === 'codex') return 'codex';
      if (p.ccAgent === 'codewhale') return 'codewhale';
      const info = paneProc.get(p.target);
      if (info?.kind) return info.kind;
      if (isClaudeCmd(p.cmd)) return 'claude';
      if (codexMatchesProcess(p.cmd)) return 'codex';
      if (codewhaleMatchesProcess(p.cmd)) return 'codewhale';
      return 'terminal';
    };
    ...
    // R11: throttled — see PANE_GC_INTERVAL_CYCLES. readPaneRegistry() above
    // still runs every cycle (it feeds this tick's pane→transcript match);
    // only the disk-cleanup pass is slowed down.
    if (this._refreshCycle % PANE_GC_INTERVAL_CYCLES === 0) {
    ...
    for (const p of claudePanes) {
      const entry = this._pins[pinKey(p.windowId, p.paneIndex)];
      const pinned = pinPath(entry);
      if (!pinned) continue;
      const pinProcStartMs = paneProc.get(p.target)?.startMs ?? null;
      if (isPinSuperseded(entry, p.paneId ? paneReg.get(p.paneId) : null, p.cwd, pinProcStartMs)) continue;
    ...
    for (const p of claudePanes) {
      ...
      const regRaw = p.paneId ? paneReg.get(p.paneId) : null;
      // Guard against a reused tmux %N: pane ids are recycled across a tmux-server
      // restart, so a record from a previous generation is a stale binding for a
      // DIFFERENT session. Judged on the record's age vs this pane's agent
      // process — NOT on cwd, which follows the agent as it cd's and so rejected
      // perfectly good records (see paneRecordBelongsToPane).
      const reg = paneRecordBelongsToPane(regRaw, p.cwd, paneProc.get(p.target)?.startMs ?? null)
        ? regRaw
        : null;
      ...
      await Promise.all(
        codexPanes.map(async (p) => {
          try {
            ...
            const rolloutPath = await this._findOpenRollout(codexPid);
            ...
            exactPaths.add(rec.transcriptPath);
            // Persist the deterministic lsof binding so an interactive Codex TUI
            // pane gets the same stable "accurate pin" a Claude pane gets from
            // record-pane.mjs. Written only on change; a new codex instance or
            // /clear in this pane opens a new rollout → this overwrites the entry
            // (new session takes over the old). No SessionEnd exists for codex,
            // but a stale entry is harmless: the exited pane becomes a shell (its
            // registry entry is no longer consulted) and a reused pane id is
            // overwritten by the next bind.
            if (p.paneId && reg?.transcriptPath !== rec.transcriptPath) {
    ...
    // One ps snapshot for the whole batch — per-session memory is the RSS of the
    // pane's process subtree (claude + MCP servers + node + …), summed below.
    const memTable = await buildProcMemTable();

    const sessions = panes.map((win) => {
      const isClaude = isClaudePane(win);
      const kind = paneKind(win);
      const hasTranscript = isClaudeKind(kind) || kind === 'codex';
      ...
      const panePrompt = this._panePromptMap.get(id) ?? null;
      // A subscribed tailer is the ONLY writer of `_pendingMap`, so a question
      // answered after the client navigated away leaves it stuck true forever.
      // The transcript already knows; let it retire the flag.
      if (shouldRetireStickyPending(transcript) && this._pendingMap.get(id)) {
      ...
      const transport = isClaudeKind(kind)
      ...
      // resolveModelLabel guards against a clipped/wrapped TUI capture (see
      // lib/tui.js) surfacing a truncated model like "Opus 4." — falls back
      // to the transcript-derived label when the TUI value looks truncated,
      // otherwise the TUI label wins (it's the only source carrying "(1M
      // context)" decoration).
      const resolvedModel = resolveModelLabel(ctx.model || null, prettyModel(transcript?.model) || null);
      // Prefer real token usage parsed from the transcript tail (see
      // computeCtxPctFromUsage above) over the TUI scrape. Codex has no
      // per-turn usage in its rollout schema here, so it — and any Claude
      // pane whose transcript hasn't logged a turn yet — falls back to the
      // TUI-scraped `ctx.ctxPct`, which itself is null when unavailable
      // (never faked).
      const transcriptCtxPct = isClaude
        ? computeCtxPctFromUsage(transcript?.contextUsedTokens ?? null, resolvedModel)
        : null;
      ...
      return {
        ...
        active: win.active,
        cwd: win.cwd,
        // RSS (MB) of this pane's whole process subtree; null when the pane's
        // pid isn't resolvable. One ps snapshot per refresh (memTable above).
        memMB: win.panePid ? sumTreeRssMB(win.panePid, memTable) : null,
        ...
        // Hard turn-end evidence (Claude JSONL) — used by _pollThinking to
        // clear a stale "working…" flag once the transcript is mtime-stable.
        turnComplete: transcript?.turnComplete ?? false,
        pending,
        pendingQuestion: transcript?.pendingQuestion ?? panePrompt?.question ?? null,
        cmd: win.cmd,
        isClaude,
        kind,
        // Harness identity and UI treatment are separate. CodeWhale's first
        // adapter is a real harness rendered through the generic PTY surface;
        // a future Runtime HTTP/SSE adapter can emit the same kind with
        // presentation:'thread'.
        presentation: kind === 'terminal' || kind === 'codewhale' ? 'terminal' : 'thread',
        transport,
        endpoint,
        ccShell: !!win.ccShell, // a composer >_ sister shell pane

        model: resolvedModel,
        ctxPct: transcriptCtxPct ?? ctx.ctxPct ?? null,
        effort: isClaude ? (ctx.effort ?? null) : null,
        ...
        subAgentActive: (isClaude || kind === 'codex') ? this._subAgentActiveMap.get(win.target) ?? false : false,
        runningSubagentCount: this._subAgentCountMap.get(win.target) ?? 0,
        workflows: (isClaude || kind === 'codex') ? this._workflowsMap.get(win.target) ?? [] : [],
        workflowActive: (isClaude || kind === 'codex') ? this._workflowActiveMap.get(win.target) ?? false : false,
        workflowSummary: (isClaude || kind === 'codex') ? this._workflowSummaryMap.get(win.target) ?? null : null,
    ...
    this._sessions = sessions.concat(this._externalRows());
   ...
   * cheap but we keep it off the hot path per the resource doctrine. Gated by
   * the same shouldScrapePane/_activeUntil predicate as _pollThinking (below)
   * so idle panes skip the capture too — see the nuance note inline.
   */
  async _pollCtx() {
    ...
    await Promise.all(
      sessions.map(async (s) => {
        ...
        try {
          ...
          // Skip idle backgrounded panes — same gate as _pollThinking (below).
          // NUANCE: ctx%/model can in theory change via an external `/model`
          // switch with no pane or transcript activity, so this gate can miss
          // that update until the pane goes active again. _pollThinking already
          // accepts this exact tradeoff for its flags, so mirroring it here for
          // ctx/model is consistent, not a new risk.
          const activeUntil = s.transcriptPath ? this._activeUntil.get(s.transcriptPath) ?? 0 : 0;
          // Backfill effort for a RECENTLY-ACTIVE Claude pane that has none yet, so
          // it can join the model/effort/ctx rotation like every other row. Bounded
          // to EFFORT_BACKFILL_WINDOW_MS since last activity: a truly-idle pane stays
          // gated (battery invariant) and a pane whose statusLine never reports effort
          // isn't re-scraped forever. Once effort is known it's sticky (below), so the
          // condition stops firing; `refreshInterval` keeps recent panes' statuslines
          // fresh so the scrape actually finds the effort.
          const needEffortBackfill =
            isClaudeKind(s.kind) &&
            !s.effort &&
            typeof s.lastActivityMs === 'number' &&
            Date.now() - s.lastActivityMs < EFFORT_BACKFILL_WINDOW_MS;
          if (
            !shouldScrapePane(s, activeUntil, Date.now(), ACTIVE_SCRAPE_WINDOW_MS) &&
            !needEffortBackfill
          )
            return;
          ...
          const { ctxPct, model, effort } =
            s.kind === 'codex' ? parseCodexTuiStatus(cap) : parseTuiStatus(cap);
          // Codex's parser has no `effort` field (its effort stays embedded in
          // the model string, parsed client-side) — effort is undefined there
          // and normalized to null so codex rows never pick up a stray value.
          // Sticky effort: a scrape that misses the effort token (idle pane still
          // showing a pre-effort statusLine render) must NOT null out an effort we
          // already have — otherwise the next full rebuild drops it and the row
          // falls out of the model/effort/ctx rotation. model/ctx survive via their
          // transcript fallbacks; effort has none, so persist the last-known here.
          const prevEffort = this._ctxMap.get(s.target)?.effort ?? null;
          // Guard against caching a truncated scrape (see lib/tui.js
          // resolveModelLabel doc): a clipped/wrapped pane capture must not
          // overwrite an already-known-good model with a half-parsed one.
          const prevModel = this._ctxMap.get(s.target)?.model ?? null;
          const resolvedScrapedModel = resolveModelLabel(model, prevModel);
          this._ctxMap.set(s.target, { ctxPct, model: resolvedScrapedModel, effort: effort ?? prevEffort });
          // Merge into the live session object without a full rebuild. For
          // Claude panes, ctxPct is normally transcript-derived (computed in
          // _doRefresh's sessions map from real message.usage, more accurate
          // than this TUI scrape) — only backfill it here when no transcript
          // usage was available at the last rebuild (s.ctxPct still null).
          // Codex has no transcript-usage source, so its TUI value always
          // applies; _ctxMap is kept fresh regardless for the next rebuild.
          if (ctxPct !== null && (!isClaudeKind(s.kind) || s.ctxPct === null)) s.ctxPct = ctxPct;
          // Same truncation guard against the live row's own last-known-good
          // model (s.model), so a clipped capture never regresses a correct
          // label already showing on the rail.
          const resolvedRowModel = resolveModelLabel(model, s.model);
          if (resolvedRowModel) s.model = resolvedRowModel;
          if (isClaudeKind(s.kind) && effort) s.effort = effort;
        } catch {
    ...
    await Promise.all(
      sessions.map(async (s) => {
        ...
        try {
          ...
          // Sub-agent activity (Claude AND Codex) — scan each session's canonical
          // spawn↔completion signals so the rail's "cloning" state lights for
          // EVERY window, not just the one a client subscribed to (the
          // SubAgentsWatcher is subscription-scoped).
          if (isClaudeKind(s.kind) || s.kind === 'codex') {
            const act = computeSubAgentActivity({ kind: s.kind, transcriptPath: s.transcriptPath });
            this._subAgentActiveMap.set(s.target, act.active);
            this._subAgentCountMap.set(s.target, act.count);
            s.subAgentActive = act.active;
            s.runningSubagentCount = act.count;

            // Workflow runs (Claude `Workflow` tool) — parse each wf_<runId>.json
            // (mtime-cached) and attach the structured slice beside the subAgent
            // fields, on the SAME poll/transport (S1). Codex sessions have no
            // workflows dir → runs is [] (cheap ENOENT).
            const runs = computeWorkflowActivity({ transcriptPath: s.transcriptPath });
            const wf = deriveWorkflowSummary(runs);
            this._workflowsMap.set(s.target, runs);
            this._workflowActiveMap.set(s.target, wf.active);
            this._workflowSummaryMap.set(s.target, wf.summary);
            s.workflows = runs;
            s.workflowActive = wf.active;
            s.workflowSummary = wf.summary;
          }

          // Pane-derived question detection (Claude AND claudex panes — both run
          // the claude binary's TUI, so parsePanePrompt scrapes both identically):
          // an on-screen numbered picker means a question is waiting — even if the
          // transcript isn't matched. This is why some sessions wrongly read as
          // "sleeping".
          if (isClaudeKind(s.kind)) {
            ...
            // Also cache the FULL parsed result (with `options`, which the
            // reduced _panePromptMap rec below deliberately drops) so
            // server.js's startPromptPoller can reuse this exact capture via
            // getPanePrompt() instead of re-running the same tmux capture-pane
            // itself (R9 dedup) — see getPanePrompt() for the freshness contract.
            this._codexPromptCache.set(s.target, { prompt, at: Date.now() });
            const rec = { pending: !!prompt, question: prompt?.question ?? null };
            this._panePromptMap.set(s.target, rec);
            const merged =
              (this._pendingMap.get(s.target) ?? false) || rec.pending || s.pending;
          ...
          // Hard-evidence idle override. A frozen "esc to interrupt"/working
          // line keeps a completed pane reading "working" indefinitely — the
          // TUI doesn't repaint once the turn ends, so the scrape re-affirms
          // thinking=true forever (and a thinking pane never idles out of the
          // scrape gate). When the transcript gives definitive turn-end
          // evidence, the file is mtime-stable, and no busy sub-state is live,
          // force the flag idle. Runs AFTER the pending/sub-agent computations
          // above so their busy states are current for the guard.
          if (s.thinking && this._turnDefinitivelyIdle(s, activeUntil)) {
            s.thinking = false;
            this._thinkingMap.set(s.target, false);
          }
        } catch {
  ...
  /**
   * Decide whether a session's "working…" flag is definitively stale and must
   * be forced idle, using hard turn-end evidence rather than the pane scrape
   * (which a frozen TUI line can keep reading "working" forever).
   *
   * ONLY Claude-family sessions (claude / claudex / claudemi, whether tmux or
   * print transport) qualify: their transcript carries a terminal stop_reason on
   * the newest main-line assistant record (`s.turnComplete`) — real proof the
   * turn ended. Codex is deliberately EXCLUDED. Codex writes a `function_call`
   * record, then goes SILENT for the entire tool/command run (a build/test can
   * exceed the mtime window by minutes), then writes `function_call_output` —
   * so transcript mtime-stability is NOT proof of idle. Force-idling Codex on it
   * would flip the rail to "idle" while it is actively working; instead Codex
   * keeps its TUI-scrape status as the sole authority.
   *
   * Gated so we never flap idle mid-stream (transcript still being written) or
   * clobber a legitimate busy sub-state (compaction, a pending question, or a
   * running sub-agent).
   *
   * @param {object} s            live session object
   * @param {number} activeUntil  fs.watch "active until" ms for this transcript (0 if none)
   * @returns {boolean}
   */
  _turnDefinitivelyIdle(s, activeUntil) {
    // Codex (and everything else) never force-idles here — see doc comment.
    if (!isClaudeKind(s.kind)) return false;
    // Never clobber a legitimate busy sub-state.
    if (s.compacting || s.pending || s.pendingQuestion || s.subAgentActive) return false;
    // mtime-stable: the transcript must have gone quiet on BOTH signals — the
    // fs.watch active window lapsed AND the last recorded activity is old. A
    // recently-written transcript may be mid-stream, so we hold off.
    const now = Date.now();
    if (activeUntil && now < activeUntil) return false;
    if (typeof s.lastActivityMs === 'number' && now - s.lastActivityMs < ACTIVE_SCRAPE_WINDOW_MS) return false;
    // Definitive turn-end from the transcript.
    return s.turnComplete === true;
  }
   ...
   */
  start() {
    if (this._interval || this._ctxInterval || this._thinkingInterval) return;
    this.refresh()
  ...
  /**
   * Safely call tmux.listWindows(). `null` means enumeration failed, while an
   * empty array is a successful observation that tmux has no panes. Keeping
   * those cases distinct prevents transient failures from pruning live state.
   *
   * @returns {Promise<import('./tmux.js').Window[]|null>}
   */
  async _listWindows() {
    try {
   ...
   * @returns {Promise<Map<string, {isClaude: boolean, isCodex: boolean, isCodeWhale?: boolean, kind: string|null, startMs: number|null, resumeSessionId?: string|null, appServer?: boolean, appServerEndpoint?: string|null}>>} target -> info
   */
  async _buildPaneProc(allPanes) {
    ...
    const findClaude = (rootPid) => {
      ...
      while (queue.length) {
        ...
        if (meta && codewhaleMatchesProcess(meta.comm, meta.args)) {
          const sec = parseEtime(meta.etime);
          return {
            isClaude: false,
            isCodex: false,
      ...
      return codexFallback || { isClaude: false, isCodex: false, isCodeWhale: false, kind: null, startMs: null, pid: null, appServer: false, appServerEndpoint: null };
    };

    for (const p of allPanes) {
      out.set(p.target, p.panePid ? findClaude(p.panePid) : { isClaude: false, isCodex: false, isCodeWhale: false, kind: null, startMs: null, pid: null, appServer: false, appServerEndpoint: null });
    }
    return out;
  }
