...
const SESSION_FORMAT = ['#{session_name}', '#{session_windows}'].join(SESSION_SEP);
...
export async function listSessions({ _run } = {}) {
  ...
  for (const line of stdout.split('\n')) {
    const trimmed = line.trim();
    if (!trimmed) continue;
    const [name, rawWindows] = trimmed.split(SESSION_SEP);
    if (!name) continue;
    sessions.push({ name, windows: Number(rawWindows) || 0 });
  }
  return sessions;
}
