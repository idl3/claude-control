/**
 * lib/tmux.js — tmux integration for claude-control.
...
/** Prefix for grouped sessions created by the PTY bridge. Kept here because
 * pane enumeration must prefer the real session when tmux lists this mirror
 * first; pty-bridge.js imports the same constant for creation and cleanup. */
...
/**
 * Resolve the tmux binary, honouring CLAUDE_CONTROL_TMUX, then absolute-path
...
export async function resolveTmuxBin() {
  ...
  // 1. Explicit override
  const envBin = process.env.CLAUDE_CONTROL_TMUX;
  if (envBin) {
    _resolvedBin = envBin;
    return _resolvedBin;
  }
  ...
  throw new Error('tmux binary not found; set CLAUDE_CONTROL_TMUX or install tmux');
}
 ...
 *
 * Only the claudex and claudemi spawn paths call this — existing spawn paths
 * are NOT gated.
 *
...
export async function assertTmuxSupportsEnv({ _run } = {}) {
  ...
  try {
    ...
    throw new Error(
      `tmux -V failed — cannot verify tmux >= 3.2 for claudex/claudemi env injection (new-session -e): ${err?.message || err}`,
    );
  }
  ...
  if (!m) {
    throw new Error(
      `could not parse tmux version from ${JSON.stringify(raw)} — tmux >= 3.2 required for claudex/claudemi env injection (new-session -e)`,
    );
  }
  ...
  if (major < 3 || (major === 3 && minor < 2)) {
    throw new Error(
      `tmux >= 3.2 required for claudex/claudemi env injection (new-session -e); found ${major}.${minor} — brew upgrade tmux`,
    );
  }
}
...
export async function listPanes() {
  ...
  return dedupePanesByPaneId(panes);
}

/**
 * Collapse panes that share a tmux pane id. Grouped sessions (e.g. a `-mobile`
 * mirror made with `new-session -t`) share the same physical panes, so
 * `list-panes -a` emits each shared pane once PER session — and the cockpit then
 * lists the same session twice. `%N` is globally unique per real pane and never
 * reused while alive, so keep the first occurrence per id. Panes with no
 * resolvable id are left as-is (nothing reliable to dedupe on). Pure + exported
 * for unit testing.
 *
 * @param {Array<{paneId: string|null}>} panes
 * @returns {Array<{paneId: string|null}>}
 */
export function dedupePanesByPaneId(panes) {
  const indexByPaneId = new Map();
  const out = [];
  for (const p of panes) {
    if (p.paneId) {
      const existingIndex = indexByPaneId.get(p.paneId);
      if (existingIndex !== undefined) {
        const existing = out[existingIndex];
        const existingIsEphemeral = existing.sessionName?.startsWith(EPHEMERAL_SESSION_PREFIX);
        const candidateIsEphemeral = p.sessionName?.startsWith(EPHEMERAL_SESSION_PREFIX);
        if (existingIsEphemeral && !candidateIsEphemeral) out[existingIndex] = p;
        continue;
      }
      indexByPaneId.set(p.paneId, out.length);
    }
    out.push(p);
  }
  return out;
...
/**
 * Kill the tmux window that hosts `target` (a session:window[.pane] ref). Used
 * by the session terminate action. Idempotent for the caller's purposes: tmux
 * errors when the window is already gone, so we swallow a "can't find" failure
 * and treat it as already-terminated.
 *
 * @param {string} target  e.g. "claude-control:4" or "claude-control:4.1"
 * @returns {Promise<{killed: boolean}>}
 */
export async function killWindow(target) {
  assertTarget(target);
  try {
    await runTmux(['kill-window', '-t', target]);
    return { killed: true };
  } catch (err) {
    if (/can't find|no such|not found/i.test(String(err?.message || err))) {
      return { killed: false }; // already gone — idempotent success
    }
    throw err;
  }
}
...
// ---------------------------------------------------------------------------
// Move window (between tmux sessions)
// ---------------------------------------------------------------------------

/**
 * Move a tmux WINDOW from its current session into a different, already-
 * existing tmux session. `srcTarget` may be a pane target
 * ("session:window.pane") or a window target ("session:window") — the pane
 * index (if any) is stripped first, since `move-window` operates on the
 * whole window, not a single pane.
 *
 * `destSession` is validated against the LIVE, raw tmux session-name list
 * (same model as `renameTmuxSession`'s `oldName` check) so a typo/stale name
 * fails loudly instead of tmux silently no-op-ing or erroring obscurely.
 *
 * The destination is addressed as `${destSession}:` (trailing colon, no
 * window index) — tmux's documented "next free index in this session" form,
 * identical to the `-t` idiom already used by `createWindow` / `splitWindow`
 * elsewhere in this file. This means a move can NEVER clobber an existing
 * window in the destination session; it always appends. Verified against a
 * real tmux 3.6a server: moving a window into a session that already
 * occupies indices 1 and 2 lands the moved window at index 3, never
 * overwriting 1 or 2.
 *
 * @param {string} srcTarget    Pane or window target of the window to move.
 * @param {string} destSession  Name of an existing tmux session to move it into.
 * @param {{ _run?: Function }} [_injected]  Test-only seam; production callers omit it.
 * @returns {Promise<void>}
 */
export async function moveWindow(srcTarget, destSession, { _run } = {}) {
  const runner = _run ?? runTmux;

  assertTarget(srcTarget);
  // move-window acts on the WINDOW, not a pane — strip a trailing ".<pane>"
  // so a pane-shaped target (e.g. "0:3.1") becomes its window ("0:3").
  const winTarget = srcTarget.replace(/\.\d+$/, '');

  // Validate against the RAW tmux session-name list, NOT listSessions() —
  // that collapses session GROUPS to a single representative (see the same
  // rationale in renameTmuxSession above).
  let names = [];
  try {
    const { stdout } = await runner(['list-sessions', '-F', '#{session_name}']);
    names = String(stdout || '').split('\n').map((s) => s.trim()).filter(Boolean);
  ...
  if (!names.includes(destSession)) {
    throw new Error(`moveWindow: no such tmux session: ${JSON.stringify(destSession)}`);
  }

  await runner(['move-window', '-s', winTarget, '-t', `${destSession}:`]);
}
