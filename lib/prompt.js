 ...
 * @returns {{ line: number, key: string, label: string, cursor: boolean, checked?: boolean }[]}
 *   Logical index of each option within the returned array is stored in `.line`.
 */
export function reconstructOptionLines(lines) {
  const matches = []; // { line, key, label, cursor, checked? }
  let i = 0;
  while (i < lines.length) {
    ...
    while (j < lines.length) {
      ...
      // Append non-empty continuation (alignment-indented wraps are intentional label continuations)
    ...
    if (label.length > MAX_LABEL) label = label.slice(0, MAX_LABEL - 1) + '…';
    matches.push({ line: matches.length, key, label, cursor, checked });
  }
  return matches;
}
 ...
 * @returns {{ question: string|undefined, multiSelect?: boolean, options: {key:string,label:string,selected:boolean,checked?:boolean}[] }|null}
 */
export function detectPanePicker(capture) {
  const stripped = stripAnsi(capture);

  // ── PICKER-OPEN detection via whitespace-collapsed blob ─────────────────────
  const blob = stripped.replace(/\s+/g, ' ').trim().toLowerCase();

  const hasAquFooter =
    AQU_FOOTER_A.test(blob) && AQU_FOOTER_B.test(blob) && AQU_FOOTER_C.test(blob);
  // Existing permission/trust/plan footers (Esc to cancel / reject / keep).
  const hasEscFooter = /\besc\b.{0,40}(cancel|reject|keep)/.test(blob);
  const hasCursorGlyph = /[❯›]/.test(stripped);

  // FALSE-POSITIVE GUARD: require at least one genuine interactive-prompt signal.
  const pickerOpen = hasAquFooter || hasEscFooter || hasCursorGlyph;
