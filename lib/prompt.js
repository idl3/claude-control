...
// Vertical box-drawing separators that split two-column bordered TUI pickers.
// Matches │ (U+2502), ┃ (U+2503), ║ (U+2551) and surrounding spaces.
const BOX_VERT_RE = /\s*[│┃║]\s*/;
// Any cell or line that consists entirely of box-drawing characters and whitespace.
// Covers the full Unicode box-drawing block U+2500–U+257F plus common ASCII rules.
// eslint-disable-next-line no-misleading-character-class
const BOX_RULE_RE = /^[\s─-╿─━—–=_┌┐└┘├┤┼┬┴╔╗╚╝╠╣╦╩╬]+$/;
...
/**
 * Normalize lines from a bordered two-column TUI picker into a flat sequence.
 *
 * A two-column picker renders as:
 *   ┌────────────────────────┬────────────────────────┐
 *   │ ❯ 1. Pause Phase B      │ 3. Push what's done     │
 *   │   2. Continue B2–B5     │      static-verified   │
 *   └────────────────────────┴────────────────────────┘
 *
 * This function:
 *   1. Splits each line on vertical box separators (│ ┃ ║) into cells.
 *   2. Drops any cell/line that is entirely box-drawing/rule characters.
 *   3. Returns the surviving cells as individual lines (left cell first, then
 *      right cell), preserving document order — so the flat sequence is:
 *      "❯ 1. Pause Phase B", "3. Push what's done", "  2. Continue B2–B5", …
 *
 * Single-column lines with no vertical separator are passed through unchanged
 * (after the pure-rule check), so non-bordered pickers are unaffected.
 *
 * @param {string[]} lines  stripped, trailing-whitespace-trimmed lines
 * @returns {string[]}      normalized flat lines
 */
function normalizeBoxLines(lines) {
  const out = [];
  for (const line of lines) {
    // Split on vertical box-drawing separators.
    const cells = line.split(BOX_VERT_RE);
    if (cells.length === 1) {
      // No vertical separator: either a single-column line or a horizontal rule.
      // Drop pure rule lines; keep everything else.
      if (!BOX_RULE_RE.test(line)) out.push(line);
    } else {
      // Multi-column: emit each non-empty, non-rule cell as its own line.
      for (const cell of cells) {
        if (cell.trim() && !BOX_RULE_RE.test(cell)) out.push(cell);
      }
    }
  }
  return out;
...
export function parsePanePrompt(capture) {
  // normalizeBoxLines splits two-column bordered pickers and drops pure rule lines
  // before any option matching occurs, so a line like "│ 2. Foo │ 3. Bar │"
  // becomes two independent candidate lines "2. Foo" and "3. Bar".
  const all = normalizeBoxLines(
