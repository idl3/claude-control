 ...
 * continuation-line stitching (hard-wrapped labels on ~22-col panes), checkbox
 * stripping, and MAX_LABEL clamping — the same logic detectPanePicker needs for
 * option-start detection. Factored out so parsePicker (lib/answer.js) can reuse
 * the same primitive instead of maintaining a parallel, narrower implementation.
 *
 ...
 *   `description` is set when deeper-indented continuation lines are found below the marker line.
 */
export function reconstructOptionLines(lines) {
  ...
  while (i < lines.length) {
    ...
    // Compute the column where the actual title text begins in the marker line.
    // This is used to distinguish AQU description lines (indented to or past the
    // title text column) from narrow-pane word-wrapped title continuations (indented
    // to the LEFT of the title text column).
    //
    // Layout of a marker line: [leading spaces] [❯/›?] [spaces] [digits] [sep] [m[3]]
    // m[3] starts at position: lines[i].length - m[3].length  (regex anchored to end)
    // If m[3] is a checkbox option "[ ] Title", the actual title starts further right.
    const m3start = lines[i].length - m[3].length;
    const m3trimmed = m[3].trim();
    const cbPre = CHECKBOX_RE.exec(m3trimmed);
    let textStartCol;
    if (cbPre) {
      // Position of title text after the checkbox: m3start + leading whitespace in m[3]
      // + offset of cbPre[2] within m3trimmed.
      const m3leading = m[3].length - m[3].trimStart().length;
      const titleOffset = m3trimmed.indexOf(cbPre[2].trim());
      textStartCol = m3start + m3leading + titleOffset;
    } else {
      // No checkbox — title text begins right after leading whitespace of m[3].
      const m3leading = m[3].length - m[3].trimStart().length;
      textStartCol = m3start + m3leading;
    }
    ...
    while (j < lines.length) {
      ...
      // Split: lines indented to or past the title text column are AskUserQuestion
      // description sub-text. Lines indented to the LEFT of the title text column are
      // narrow-pane word-wrapped label continuations (the title overflowed the line).
      const contIndent = nextLine.match(/^\s*/)[0].length;
      if (contIndent >= textStartCol) {
        // At or past title text column → description (sub-text below the option title).
        descParts.push(nextLine.trim());
      } else {
        // Left of title text column → fold into label (narrow-pane title-wrap continuation).
        label = (label + ' ' + nextLine.trim()).replace(/\s+/g, ' ').trim();
      }
      j++;
    }
    i = j;
...
export function detectPanePicker(capture) {
  ...
  const FOOTER_REGION = 14; // physical lines from the bottom that hold an active footer
  const footerBlob = stripped
    .split('\n')
    .slice(-FOOTER_REGION)
    .join(' ')
    .replace(/\s+/g, ' ')
