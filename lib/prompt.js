...
export function parsePanePrompt(capture) {
  ...
  // Anchor on the LAST "1." (the bottom-most menu), then walk forward taking the
  // next-expected key (2, 3, …), skipping any interleaved description/prose.
  let anchor = -1;
  ...
  const options = [];
  let expected = 1;
  for (let j = anchor; j < matches.length; j++) {
    if (Number(matches[j].key) === expected) {
      options.push(matches[j]);
      expected += 1;
    }
  }
