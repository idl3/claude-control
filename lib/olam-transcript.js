...
export class ShapeSubscriber extends EventEmitter {
  ...
  /**
   * Long-poll loop until stop(). Emits 'append' (messages), 'ready' (initial
   * snapshot drained to the live cursor — fires exactly once, for BOTH empty
   * and non-empty transcripts), and 'degraded' (reason).
   */
  async start() {
    ...
    try {
      ...
      // Snapshot fully drained (and not torn down mid-drain) — the caller can
      // now trust "no messages yet" as a genuine empty transcript rather than
      // an in-flight backfill. Fires once per start(), before the live tail.
      if (this._running) this.emit('ready');
      // Live long-poll tail.
      while (this._running) {
 ...
 * "log tail only" state, and a `ready` event once the initial shape snapshot
 * has drained to the live cursor (fires for a genuinely-empty transcript too —
 * see server.js's `olam-transcript-ready` WS forward + useClaudeControl's
 * remote-only `messagesLoaded` gate). Full mode = Electric chunks shape; on
 * shape auth/network failure it transparently falls back to polling the
 * runner feed (which does not emit `ready` — the client's 8s loader timeout
