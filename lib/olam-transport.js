 ...
 * dispatchSteer.
 *
 * A 2xx does NOT by itself prove the steer was LIVE-delivered. The steer door
 * persists the instruction on the durable ledger and, in soft mode, mirrors it
 * onto the live in-container agent — but when the agent has died / was never
 * dispatched, the SPA returns 202 with `liveSteerRelay: { ok:false,
 * mirrored:false, error:'session_never_dispatched' }` (or `hardSteerRelay` for
 * hard mode): the row was queued but NOT applied live. We replicate the
 * plan-chat-spa client's proven derivation (writeSessionSteer, chunks.ts):
 *   `mirrored = relay.ok === true && relay.mirrored !== false`
 * so the caller can tell a truly-live steer from a merely-queued one. Fail-soft:
 * a body-less / unparseable 2xx, or one carrying no relay object at all, keeps
 * `mirrored: true` — we never REGRESS a genuinely-delivered steer into a false
 * "not applied" warning.
 *
 * @param {import('./olam-client.js').OlamOrgClient} client
 * @param {{ sessionId: string, draft: string, mode?: 'soft'|'hard' }} args
 * @returns {Promise<{ ok: true, status: number, mirrored: boolean } | { ok: false, status: number|null, error: string }>}
 */
async function steerDoorPost(client, { sessionId, draft, mode = 'soft' }) {
  ...
  if (res.ok) {
    // Default true so an un-parseable / relay-less 2xx never regresses a
    // genuinely-delivered steer into a false warning (fail-soft).
    let mirrored = true;
    try {
      const data = await res.json();
      const relay = data?.liveSteerRelay ?? data?.hardSteerRelay;
      if (relay) mirrored = relay.ok === true && relay.mirrored !== false;
    } catch {
      // no JSON body / no json() method — keep mirrored:true.
    }
    return { ok: true, status: res.status, mirrored };
  }
 ...
 * send. On the `'steer-live'` branch the result also carries `mirrored`
 * (steerDoorPost's true-delivery signal): `false` means the steer was queued
 * on the ledger but NOT applied to a live agent, so the caller can ack an
 * honest "queued, not yet applied" message instead of a plain success. The
 * `'dispatch'` branch has no `mirrored` field (cloud-dispatch's 202 is a
 * legitimately-dispatched new turn with no relay to inspect) — that's fine.
 *
 * @param {import('./olam-client.js').OlamOrgClient} client
 ...
 * @returns {Promise<{ ok: true, status: number, door: 'steer-live'|'dispatch', mirrored?: boolean } | { ok: false, status: number|null, error: string, door: 'steer-live'|'dispatch' }>}
 */
 ...
 * @returns {'olam'|'codewhale-runtime'|'claude-print'|'codex-rpc'|'tmux'}
 */
export function replyTransport(session) {
  if (session?.kind === 'remote') return 'olam';
  if (session?.kind === 'codewhale' && session?.transport === 'codewhale-http-sse') {
    return 'codewhale-runtime';
  }
