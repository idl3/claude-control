 ...
 * @property {string|null} [resumeSessionId] session UUID pulled from `claude --resume <uuid>`
 *                          process args (see resumeSessionIdFromArgs in sessions.js).
 *                          When set, a cwd-consistent candidate with the same id binds
 *                          FIRST, bypassing the mtime temporal guard (resume-idle fix).
 *
 ...
 * @property {string|null} [sessionId]    session id recorded inside the transcript (falls back
 *                          to the transcript basename when absent) — used for resume-id binding
 */
 ...
 *   0. Resume-id exact bind — a pane launched with `claude --resume <uuid>`
 *      carries that session id (pane.resumeSessionId). A cwd-consistent candidate
 *      whose recorded sessionId / basename equals it binds FIRST, bypassing the
 *      temporal guard so a resumed-but-idle transcript (old mtime) is recovered.
...
export function assignTranscripts(panes, candidates, opts = {}) {
  ...
  // Pass 0 — resume-id exact bind (highest priority, bypasses the temporal
  // guard). A pane launched with `claude --resume <id>` carries the resumed
  // session's UUID in its process args, extracted upstream into
  // pane.resumeSessionId. If a cwd-consistent candidate's recorded sessionId —
  // or, as a fallback, its transcript basename — equals that UUID, bind it
  // DIRECTLY, even when the transcript's last activity predates the pane's
  // process start.
  //
  // Why bypass temporallyPlausible here: a resumed-but-idle session's transcript
  // keeps its OLD mtime (no new records until the user actually sends a message),
  // so the temporal guard rejects it as a stale leftover — the resume-idle bug
  // (empty chat for a live resumed pane). The UUID is a zero-ambiguity signal
  // (globally unique, read straight from the live process), so skipping the guard
  // is safe. Only an EXPLICIT id match does so: a fresh session has no resume id,
  // so Pass 0 no-ops and it still gets no stale sibling bound (guard preserved).
  const candidateSessionId = (c) => {
    if (c.sessionId) return String(c.sessionId).toLowerCase();
    const base = String(c.transcriptPath || '').split('/').pop() || '';
    const stripped = base.replace(/\.jsonl$/i, '');
    return stripped ? stripped.toLowerCase() : null;
  };
  for (const pane of ordered) {
    if (result.has(pane.target)) continue;
    const wanted = pane.resumeSessionId ? String(pane.resumeSessionId).toLowerCase() : null;
    if (!wanted) continue;
    for (const c of available(pane)) {
      if (candidateSessionId(c) === wanted) {
        claim(pane, c);
        break;
      }
    }
