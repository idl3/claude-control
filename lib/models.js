 ...
 *
 * Exports:
 *  - MLX_MODELS, CLAUDE_MODELS, CODEX_MODELS, CLAUDEX_MODELS, CLAUDEMI_MODELS  (catalogs)
 ...
 * ORDER MATTERS: the first entry is the picker's "Default" row (see
 * NewSessionDraft's `modelOptions[0]`), so the newest flagship leads.
...
/**
 * Claudex models — the claude CLI pointed at the olam auth-worker (which
 * translates Anthropic-shaped requests to OpenAI). Deliberately a closed
 * single-entry list: the auth-worker serves a fixed model set, and the cockpit
 * offers no generic custom-base-URL/model configurability (design rubric S3).
 * Single source of truth: server.js derives ALLOWED_CLAUDEX_MODELS from this
 * list, and the SPA fetches it via /api/models rather than hardcoding a copy.
 * @type {{ id: string, label: string }[]}
 */
export const CLAUDEX_MODELS = [
  { id: 'gpt-5.6-sol', label: 'GPT-5.6 Sol (Codex)' },
];

/**
 * Claudemi models — the claude CLI pointed at the olam auth-worker's `/kimi`
 * provider-selector segment (which routes Anthropic-shaped requests to Kimi).
 * Closed two-entry list, mirroring CLAUDEX_MODELS' single-source-of-truth
 * pattern: server.js derives ALLOWED_CLAUDEMI_MODELS from this list, and the
 * SPA fetches it via /api/models rather than hardcoding a copy.
 * @type {{ id: string, label: string }[]}
 */
export const CLAUDEMI_MODELS = [
