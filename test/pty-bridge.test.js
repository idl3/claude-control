...
import { createPtyBridge, handlePtyUpgrade, PTY_CHANNEL_DATA } from '../lib/pty-bridge.js';
...
/** Flush pending microtasks so injected-promise chains (spawn/wait/etc) settle. */
...
test('LRU cap evicts the idle attach when MAX is exceeded', async () => {
  ...
  assert.equal(handles['a:1'].killed, true, 'the idle LRU victim was killed');
  assert.equal(handles['b:1'].killed, false, 'the still-attached session survives eviction');
