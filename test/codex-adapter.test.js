...
test('matchesProcess: true for codex', () => {
  assert.equal(CodexAdapter.matchesProcess('codex'), true);
});

test('matchesProcess: true for /usr/local/bin/codex', () => {
  assert.equal(CodexAdapter.matchesProcess('/usr/local/bin/codex'), true);
});

test('matchesProcess: true for codex --foo', () => {
  assert.equal(CodexAdapter.matchesProcess('codex --foo'), true);
});

test('matchesProcess: false for codex-control', () => {
  assert.equal(CodexAdapter.matchesProcess('codex-control'), false);
});

test('matchesProcess: false for 2.1.162', () => {
  assert.equal(CodexAdapter.matchesProcess('2.1.162'), false);
});

test('matchesProcess: false for zsh', () => {
  assert.equal(CodexAdapter.matchesProcess('zsh'), false);
});

test('matchesProcess: false for empty string', () => {
  assert.equal(CodexAdapter.matchesProcess(''), false);
});
...
test('discovery: indexes sample-rollout.jsonl by cwd, sets agentType=codex', async () => {
  ...
  try {
    const index = await CodexAdapter.buildTranscriptIndex({ codexSessionsRoot: temp }, now);
...
test('discovery resilience: empty file and bad JSON do not throw, index nothing', async () => {
  ...
  await assert.doesNotReject(async () => {
    index = await CodexAdapter.buildTranscriptIndex({ codexSessionsRoot: temp }, now);
  });
...
// ---------------------------------------------------------------------------
// 4. parseRecord — per-type assertions
// ---------------------------------------------------------------------------
...
test('parseRecord: assistant message → role assistant, text block', () => {
  ...
  const msg = CodexAdapter.parseRecord(line);
...
test('parseRecord: user message → role user, text block', () => {
  const line = JSON.stringify({
    type: 'response_item',
    payload: {
      type: 'message',
...
test('parseRecord: developer message → null (system injection filtered)', () => {
  const line = JSON.stringify({
    type: 'response_item',
    payload: {
      type: 'message',
  ...
  assert.equal(CodexAdapter.parseRecord(line), null);
});

test('parseRecord: reasoning → role assistant, thinking block [reasoning encrypted]', () => {
  const line = JSON.stringify({
    type: 'response_item',
    payload: {
      type: 'reasoning',
...
test('parseRecord: function_call → tool_use block with id, name, input, inputSummary', () => {
  const line = JSON.stringify({
    type: 'response_item',
    payload: {
      type: 'function_call',
...
test('parseRecord: function_call_output → tool_result block', () => {
  const line = JSON.stringify({
    type: 'response_item',
    payload: {
...
test('parseRecord: custom_tool_call apply_patch → tool_use name=apply_patch, input.patch has Begin Patch', () => {
  const line = JSON.stringify({
    type: 'response_item',
    payload: {
...
test('parseRecord: custom_tool_call_output → tool_result forId=call_id', () => {
  const line = JSON.stringify({
    type: 'response_item',
    payload: {
...
test('parseRecord: event_msg/agent_message → null', () => {
  const line = JSON.stringify({
    type: 'event_msg',
...
test('parseRecord: event_msg/token_count → null', () => {
  const line = JSON.stringify({
    type: 'event_msg',
...
test('parseRecord: turn_context → null', () => {
  const line = JSON.stringify({
    type: 'turn_context',
...
test('parseRecord: session_meta → null', () => {
  const line = JSON.stringify({
    type: 'session_meta',
...
test('parseRecord: call_id linkage — function_call id === function_call_output forId (fixture lines 12+14)', () => {
  const fcMsg = CodexAdapter.parseRecord(fcLine);
  const fcoMsg = CodexAdapter.parseRecord(fcoLine);
...
test('pending exec: transcriptPending=true, kind=exec_command, 3 options, option[0] highlighted', () => {
  const result = CodexAdapter.detectPendingFromCapture(execApprovalCapture);
...
test('pending edit: transcriptPending=true, kind=apply_patch, 3 options, option1 highlighted, option2 label includes "don\'t ask again"', () => {
  const result = CodexAdapter.detectPendingFromCapture(editApprovalCapture);
  assert.equal(result.transcriptPending, true);
...
test('pending none: capture with no heading → transcriptPending=false', () => {
  ...
  const result = CodexAdapter.detectPendingFromCapture(capture);
...
test('buildAnswerProgram: explicit digit selection → [digit, Enter]', () => {
  const pending = CodexAdapter.detectPendingFromCapture(execApprovalCapture);
  const result = CodexAdapter.buildAnswerProgram(pending, [['1']]);
  assert.deepEqual(result, ['1', 'Enter']);
});

test('buildAnswerProgram: empty selections → default to highlighted option 1 → [1, Enter]', () => {
  const pending = CodexAdapter.detectPendingFromCapture(execApprovalCapture);
  const result = CodexAdapter.buildAnswerProgram(pending, []);
  assert.deepEqual(result, ['1', 'Enter']);
});

test('buildAnswerProgram: label selection "Yes, proceed" → [1, Enter]', () => {
  const pending = CodexAdapter.detectPendingFromCapture(execApprovalCapture);
  const result = CodexAdapter.buildAnswerProgram(pending, [['Yes, proceed']]);
  assert.deepEqual(result, ['1', 'Enter']);
});
