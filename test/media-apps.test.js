...
// ── listVersions + route level (_handler) ──────────────────────────────────
//
// Wrapped in an async describe() rather than bare top-level test() calls
// sharing a module-scope `await import(...)`. node:test's top-level test()
// registration can start EXECUTING already-registered siblings while the
// module is still suspended on that `await` (observed directly: growing
// server.js's own import graph — e.g. A4's lib/pty-bridge.js addition, which
// pulls in no new runtime behavior on this path, just more modules to link —
// was enough to tip the race and fire the file-scope after() hook after only
// ~5 of 18 tests had run, deleting `root` while later tests still needed it).
// describe()'s callback is fully awaited BEFORE any of its nested tests run,
// so hooks declared inside it are unambiguously scoped to its own children
// regardless of import-graph size or Node version — the actual fix, not a
// timing workaround.
describe('listVersions + route (fixture-backed)', async () => {
